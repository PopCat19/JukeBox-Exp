...
export function renderTimeline(
  ...
  if (zoomEnabled) {
    ...
    windowOctaves = Math.max(
      3,
      Math.min(ui.synth.song.octaveCount, Math.round(timelineHeight / (12 * targetSemitoneHeight))),
    );
    windowPitchCount = windowOctaves * 12 + 1;
  }
  ...
  for (let bar: number = 0; bar < ui.synth.song.barCount + 1; bar++) {
    const color: string = (bar == ui.synth.song.loopStart || bar == ui.synth.song.loopStart + ui.synth.song.loopLength)
