...
export function drawNote(pitch: number, start: number, pins: NotePin[], radius: number, offsetX: number, offsetY: number, partWidth: number, pitchHeight: number): string {
...
export function renderTimeline(ui: PlayerUI, zoomEnabled: boolean, removeFromUnorderedArray: <T>(array: T[], index: number) => void): void {
	...
	for (let bar: number = 0; bar < ui.synth.song.barCount + 1; bar++) {
		const color: string = (bar == ui.synth.song.loopStart || bar == ui.synth.song.loopStart + ui.synth.song.loopLength) ? ColorConfig.loopAccent : ColorConfig.uiWidgetBackground;
	...
	for (let octave: number = 0; octave <= windowOctaves; octave++) {
			ui.timeline.appendChild(rect({x: 0, y: octave * 12 * wavePitchHeight, width: timelineWidth, height: wavePitchHeight + 1, fill: ColorConfig.tonic, opacity: 0.75}));
	}
	...
	for (let channel: number = ui.synth.song.channels.length - 1 - ui.synth.song.modChannelCount; channel >= 0; channel--) {
		...
		const newOctaveScroll: number = Math.max(0, Math.min(ui.synth.song.octaveCount - windowOctaves, Math.ceil(configuredOctaveScroll - windowOctaves * 0.5)));
			...
			for (let i: number = 0; i < pattern.notes.length; i++) {
				...
				for (const pitch of note.pitches) {
					const d: string = drawNote(pitch, note.start, note.pins, (pitchHeight + 1) / 2, offsetX, offsetY, partWidth, pitchHeight);
						const noteElement: SVGPathElement = path({d: d, fill: ColorConfig.getChannelColor(ui.synth.song, channel).primaryChannel});
