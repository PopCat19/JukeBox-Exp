...
export class PlayerControls {
  ...
  private onTimelineCursorMove(mouseX: number): void {
    if (this.draggingPlayhead && this.ui.synth.song != null) {
      ...
      this.ui.synth.playhead = this.ui.synth.song.barCount * (mouseX - boundingRect.left)
  ...
  private renderLoopIcon(): void {
    this.ui.loopIcon.setAttribute(
      "fill",
      (this.ui.synth.loopRepeatCount == -1) ? ColorConfig.linkAccent : ColorConfig.uiWidgetBackground,
    );
  }
  ...
  public shortenUrl() {
    ...
    window.open(
      shortenerStrategy + encodeURIComponent(new URL("#" + this.ui.synth.song!.toBase64String(), location.href).href),
    );
  }
  ...
  private updateSampleLoadingBar(_e: Event): void {
    ...
    this.ui.sampleLoadingBarContainer.title = "Total Samples: " + String(e.totalSamples) + "; Loaded Samples: "
      + String(e.samplesLoaded) + "; ";
