...
export function buildPlayerUI(): PlayerUI {
  ...
  const playhead: HTMLDivElement = div({
    style:
      `position: absolute; left: 0; top: 0; width: 2px; height: 100%; background: ${ColorConfig.playhead}; pointer-events: none;`,
  });
  ...
  const sampleLoadingBarContainer: HTMLDivElement = div({
    ...
    style:
      `overflow: hidden; margin: auto; width: 90%; height: 50%; background-color: var(--empty-sample-bar, ${ColorConfig.indicatorSecondary});`,
    preserveAspectRatio: "none",
  }, sampleLoadingBar);
  const sampleLoadingStatusContainer: HTMLDivElement = div(
    {},
    div(
      { class: "selectRow", style: "overflow: hidden; margin: auto; width: 160px; height: 10px; " },
