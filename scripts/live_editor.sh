...
# Uses the "concurrently" package to execute multiple tasks simultaneously.
...
# will be generated automatically. Finally, the "five-server" package is
...
bunx concurrently \
	"bunx tsc -p scripts/tsconfig_editor.json --watch --preserveWatchOutput" \
	"bunx rollup build/editor/main.js --file website/beepbox_editor.js --format iife --output.name beepbox --context exports --sourcemap --plugin rollup-plugin-sourcemaps --plugin @rollup/plugin-node-resolve --watch.buildDelay 200 -w" \
