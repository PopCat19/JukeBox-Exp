...
export class CustomAlgorythmCanvas {
  ...
  constructor(
    ...
    private readonly _getChange: (newArray: number[][], carry: number, mode: string) => Change,
  ) {
  ...
  public fillDrawArray(noReset: boolean = false): void {
    if (noReset) {
      ...
      const oldMods =
        this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].customAlgorithm;
      ...
      if (this.mode == "feedback") {
        ...
        const oldfeed =
          this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].customFeedbackType
            .indices;
      ...
      if (i < this.carriers) {
        ...
        if (this.inverseModulation[i][0] != undefined) {
          const testPos = [
            ...
            - (this.lookUpArray[this.inverseModulation[i][this.inverseModulation[i].length - 1] - 1][0] + 2),
