...
function buildPresetOptions(isNoise: boolean, idSet: string): HTMLSelectElement {
  ...
  if (isNoise) {
    for (const plugin of getRegisteredPlugins()) {
      if (
        plugin.type == InstrumentType.noise || plugin.type == InstrumentType.spectrum
...
export class SongEditor implements ModSliderProvider {
  ...
  private readonly _instrumentVolumeSliderTip: HTMLDivElement = div(
    ...
    span(
      { class: "tip", style: "font-size: smaller;", onclick: () => this._openPrompt("instrumentVolume") },
      "Volume: ",
    ),
  );
  ...
  private readonly _useChipWaveAdvancedLoopControlsRow: HTMLDivElement = div(
    { class: "selectRow" },
    span(
      { class: "tip", style: "flex-shrink: 0;", onclick: () => this._openPrompt("loopControls") },
      "Loop Controls: ",
    ),
  ...
  private readonly _clicklessTransitionRow: HTMLElement = div(
    { class: "selectRow dropFader" },
    span(
      { class: "tip", style: "margin-left:4px;", onclick: () => this._openPrompt("clicklessTransition") },
      "‣ Clickless:",
    ),
  ...
  private readonly _twoNoteArpRow: HTMLElement = div(
    { class: "selectRow dropFader" },
    span(
      { class: "tip", style: "margin-left:4px;", onclick: () => this._openPrompt("twoNoteArpeggio") },
      "‣ Fast Two-Note:",
    ),
    this._twoNoteArpBox,
  );
  ...
  private readonly _customInstrumentSettingsGroup: HTMLDivElement = div(
    ...
    div(
      ...
      span(
        { style: `flex-grow: 1; text-align: center;` },
        span({ class: "tip", onclick: () => this._openPrompt("envelopes") }, "Envelopes"),
      ),
  ...
  private readonly _sampleLoadingBarContainer: HTMLDivElement = div({
    style:
      `width: 80%; height: 4px; overflow: hidden; margin-left: auto; margin-right: auto; margin-top: 0.5em; cursor: pointer; background-color: ${ColorConfig.indicatorSecondary};`,
  ...
  constructor(/*private _doc: SongDocument*/) {
    ...
    for (let i: number = 0; i < Config.operatorCount + 2; i++) {
      ...
      const waveformPulsewidthSlider: Slider = new Slider(
        ...
        this.doc,
        (oldValue: number, newValue: number) =>
          new ChangeOperatorPulseWidth(this.doc, operatorIndex, oldValue, newValue),
        true,
      );
    ...
    for (let mod: number = 0; mod < Config.modCount; mod++) {
      ...
      this._modulatorGroup.appendChild(
        div({
          style: "margin: 3px 0; font-weight: bold; margin-bottom: 0.7em; text-align: center; color: "
            + ColorConfig.secondaryText + "; background: " + ColorConfig.uiWidgetBackground + ";",
    ...
    this._useChipWaveAdvancedLoopControlsBox.addEventListener(
      "input",
      this._dispatch.whenSetUseChipWaveAdvancedLoopControls,
    );
      ...
      this.doc.record(
        new ChangeUnisonVoices(
          ...
          Math.min(
            ...
            Math.max(Config.unisonVoicesMin, Math.round(+this._unisonVoicesInputBox.value)),
          ),
        ),
      );
    });
      ...
      this.doc.record(
        new ChangeUnisonExpression(
          ...
          Math.min(
            ...
            Math.max(Config.unisonExpressionMin, +this._unisonExpressionInputBox.value),
          ),
        ),
      );
    });
  ...
  private _updateSampleLoadingBar(_e: Event): void {
    ...
    const percent: number = e.totalSamples === 0
      ? 0
    ...
    switch (dropdown) {
      ...
      case DropdownID.EnvelopeSettings:
        ...
        this.envelopeEditor.openExtraSettingsDropdowns[submenu] =
          this.envelopeEditor.openExtraSettingsDropdowns[submenu] ? false : true;
      ...
      if (anyModActive) {
        ...
        function updateModSlider(
          ...
          if (editor.doc.synth.isModActive(setting, channel, instrument)) {
            ...
            let currentVal: number = (editor.doc.synth.getModValue(setting, channel, instrument, false)
  ...
  private _onFocusIn = (event: Event): void => {
    if (
      this.doc.synth.recording && event.target != this.mainLayer && event.target != this._stopButton
  ...
  private _renderInstrumentBar(channel: Channel, instrumentIndex: number, colors: ChannelColors) {
    if (this.doc.song.layeredInstruments || this.doc.song.patternInstruments) {
      ...
      this._instrumentRemoveButton.style.display = (channel.instruments.length > Config.instrumentCountMin)
        ? ""
        : "none";
      ...
      if (
        this.doc.song.layeredInstruments && this.doc.song.patternInstruments
      ...
      if (
        (this.doc.song.layeredInstruments && this.doc.song.patternInstruments) && channel.instruments.length > 1
        && (this.doc.channel < this.doc.song.pitchChannelCount + this.doc.song.noiseChannelCount)
      ) {
        for (let i: number = 0; i < channel.instruments.length; i++) {
  ...
  public updatePlayButton = (): void => {
    if (
      this._renderedIsPlaying != this.doc.synth.playing || this._renderedIsRecording != this.doc.synth.recording
      || this._renderedShowRecordButton != this.doc.prefs.showRecordButton || this._renderedCtrlHeld != this._ctrlHeld
    ) {
      ...
      if (
        document.activeElement == this._playButton || document.activeElement == this._pauseButton
        || document.activeElement == this._recordButton || document.activeElement == this._stopButton
      ) {
  ...
  private _usageCheck(channelIndex: number, instrumentIndex: number): void {
    ...
    if (channel.bars[this.doc.bar] != 0) {
      for (let i: number = 0; i < this.doc.song.barCount; i++) {
        ...
        if (
          channel.bars[i] == channel.bars[this.doc.bar] && i != this.doc.bar
    ...
    for (let i: number = 0; i < this.doc.song.barCount; i++) {
      ...
      if (
        channel.bars[i] != 0 && channel.bars[i] != channel.bars[this.doc.bar]
  ...
  private _fileMenuHandler = (event: Event): void => {
    switch (this._fileMenu.value) {
      ...
      case "shortenUrl":
        ...
        window.open(
          shortenerStrategy + encodeURIComponent(new URL("#" + this.doc.song.toBase64String(), location.href).href),
        );
        break;
      case "configureShortener":
  ...
  private _customWavePresetHandler = (event: Event): void => {
    ...
    for (let i: number = 0; i < 64; i++) {
      ...
      customWaveArray[i] = (Config.chipWaves[index].samples[Math.floor(arrayPoint)]
        - Config.chipWaves[index].samples[Math.floor(arrayPoint) + 1]) / arrayStep;
