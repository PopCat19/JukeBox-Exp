...
export function showNpcDialogue(s: SummonedShiggy): void {
    ...
    positionBubble(
        bubble, s.x, s.y,
        parseInt(s.img.style.zIndex || "9999", 10),
    );
    setTimeout(() => {
...
export function startConversation(
    ...
    const exchange =
        CONVO_EXCHANGES[Math.floor(Math.random() * CONVO_EXCHANGES.length)];
...
function _playConvoTurn(
    ...
    const text =
        turn < lines.length
            ...
            : CONVO_RESPONSES[
                  Math.floor(Math.random() * CONVO_RESPONSES.length)
              ][turn % 2];
    ...
    positionBubble(
        bubble, speaker.x, speaker.y,
        parseInt(speaker.img.style.zIndex || "9999", 10),
    );
...
function _endConversation(s: SummonedShiggy): void {
    if (s.convoPartner) {
        ...
        p.convoPartner = null;
        if (p.convoTimer) { clearTimeout(p.convoTimer); p.convoTimer = null; }
    }
    clearDialogue(s);
    s.inConversation = false;
    s.convoPartner = null;
    if (s.convoTimer) { clearTimeout(s.convoTimer); s.convoTimer = null; }
}
...
export function tickGroupDialogue(
    ...
    for (let i = 0; i < summoned.length; i++) {
        ...
        for (let j = 0; j < summoned.length; j++) {
            ...
            const b = summoned[j];
            const dx =
                a.x + SHIGGY_SIZE / 2 - (b.x + SHIGGY_SIZE / 2);
            const dy =
                a.y + SHIGGY_SIZE / 2 - (b.y + SHIGGY_SIZE / 2);
...
function _fireGroupBubble(s: SummonedShiggy, now: number): void {
    clearDialogue(s);
    const text =
        GROUP_LINES[Math.floor(Math.random() * GROUP_LINES.length)];
