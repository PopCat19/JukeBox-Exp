...
export function spawnShiggy(onPop: () => void): SummonedShiggy {
  ...
  img.style.cssText =
    `width: ${SHIGGY_SIZE}px; height: auto; pointer-events: none; opacity: 0; position: fixed; z-index: 9999; image-rendering: auto;`;
  ...
  requestAnimationFrame(() => {
    img.style.opacity = "1";
    img.style.animation =
      `shiggy-summon-enter 0.4s ease-out both, shiggy-float ${floatDuration}s ease-in-out 0.4s infinite`;
  });
