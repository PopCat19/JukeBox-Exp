...
export class MuteEditor {
	...
	private readonly _channelDropDown: HTMLSelectElement = HTML.select(
		...
		HTML.option({ value: "chnCleanInst" }, "Clean Instruments (LSDj)"),
	...
	public render(): void {
		...
		for (let y: number = 0; y < this._doc.song.getChannelCount(); y++) {
			...
			if (active && !playing) {
				...
				// 2.51x gain maps -8dB (amp 0.398) to v=1.0; ref=1.0 means the
				// compression saturates around -8dB but approaches gradually.
				const v = (this._channelPeak[y] ?? 0) * 2.51;
