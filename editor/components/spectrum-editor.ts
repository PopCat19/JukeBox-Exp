...
export class SpectrumEditor {
    ...
    for (let i: number = 0; i < Config.spectrumControlPoints; i += Config.spectrumControlPointsPerOctave) {
      this._octaves.appendChild(
        SVG.rect({
          ...
          x: (i + 1) * this._editorWidth / (Config.spectrumControlPoints + 2) - 1,
          y: 0,
          width: 2,
  ...
  private _whenCursorMoved(): void {
    if (this._mouseDown) {
      ...
      const spectrumWave: SpectrumWave = (this._spectrumIndex == null)
      ...
      spectrumWave.spectrum[Math.max(0, Math.min(Config.spectrumControlPoints - 1, Math.round(freq)))] = Math.max(
        0,
        Math.min(Config.spectrumMax, Math.round(amp)),
      );
  ...
  public setSpectrumWave(spectrum: number[], saveHistory: boolean = false) {
    ...
    if (this._spectrumIndex == null) {
      ...
      const spectrumChange: ChangeSpectrum = new ChangeSpectrum(
        this._doc,
        instrument,
        instrument.drumsetSpectrumWaves[this._spectrumIndex],
      );
  ...
  public render(): void {
    ...
    for (let i: number = 0; i < Config.spectrumControlPoints; i++) {
      ...
      path += prettyNumber((i + 1) * this._editorWidth / (Config.spectrumControlPoints + 2)) + " "
    ...
    if (this._renderedPath != path) {
      ...
      this._fill.setAttribute(
        "d",
        path + "L " + this._editorWidth + " " + prettyNumber(lastHeight) + " L " + this._editorWidth + " "
          + prettyNumber(this._editorHeight) + " L 0 " + prettyNumber(this._editorHeight) + " z ",
      );

      this._arrow.setAttribute(
        "d",
        "M " + this._editorWidth + " " + prettyNumber(lastHeight) + " L " + (this._editorWidth - 4) + " "
          + prettyNumber(lastHeight - 4) + " L " + (this._editorWidth - 4) + " " + prettyNumber(lastHeight + 4) + " z",
      );
...
export class SpectrumEditorPrompt implements Prompt {
  ...
  public readonly _drumsetButtonContainer: HTMLDivElement = HTML.div({
  ...
  constructor(private _doc: SongDocument, private _songEditor: SongEditor, private _isDrumset: boolean) {
    ...
    if (this._isDrumset) {
      for (let i: number = Config.drumCount - 1; i >= 0; i--) {
        ...
        this.spectrumEditors[i].setSpectrumWave(
          this._songEditor._drumsetSpectrumEditors[Config.drumCount - 1 - i].getSpectrumWave().spectrum,
        );
      }
      const colors = ColorConfig.getChannelColor(this._doc.song, this._doc.channel);
      for (let i: number = 0; i < Config.drumCount; i++) {
        const newSpectrumButton: HTMLButtonElement = HTML.button(
          { class: "no-underline", style: "max-width: 2em;" },
          "" + (i + 1),
        );
