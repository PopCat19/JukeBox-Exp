...
export function renderInstrumentBar(
    ...
    if (doc.song.layeredInstruments || doc.song.patternInstruments) {
        ...
        if ((doc.song.layeredInstruments && doc.song.patternInstruments) && channel.instruments.length > 1 && (doc.channel < doc.song.pitchChannelCount + doc.song.noiseChannelCount)) {
            for (let i: number = 0; i < channel.instruments.length; i++) {
