...
export class FilterEditor {
    ...
    if (this._larger) {
      ...
      for (let i: number = 1; i < Config.filterMorphCount; i++) {
        if (this._forSong) {
          ...
          const subFilter: FilterSettings | null = this._useNoteFilter
            ? instrument.noteSubFilters[i]
            : instrument.eqSubFilters[i];
          if (subFilter != null) {
            const parsedFilter: FilterSettings = new FilterSettings();
            parsedFilter.fromJsonObject(subFilter.toJsonObject());
            this._subFilters[i] = parsedFilter;
  ...
  private _updateCursor(): void {
    ...
    for (let i: number = 0; i < this._useFilterSettings.controlPointCount; i++) {
      ...
      const distance: number = Math.sqrt(
        Math.pow(this._freqToX(point.freq) - this._mouseX, 2) + Math.pow(this._gainToY(point.gain) - this._mouseY, 2),
      );
      if (
        (distance <= 13 * (1 + +this._larger) || this._useFilterSettings.controlPointCount >= Config.filterMaxPoints)
  ...
  private _whenCursorMoved(): void {
    if (this._writingMods) {
      ...
      if (this._dragChange != null) {
        if (
          this._dragChange instanceof ChangeSequence && this._dragChange.checkFirst() instanceof ChangeFilterMovePoint
        ) {
          const data: FilterMoveData = ((this._dragChange as ChangeSequence).checkFirst() as ChangeFilterMovePoint)
            .getMoveData(true);
          ...
          this._forSong && this._dragChange instanceof ChangeSequence
          && this._dragChange.checkFirst() instanceof ChangeSongFilterMovePoint
        ) {
          const data: FilterMoveData = ((this._dragChange as ChangeSequence).checkFirst() as ChangeSongFilterMovePoint)
            .getMoveData(true);
          const newPoint: FilterControlPoint | null = this._useFilterSettings.controlPoints[this._selectedIndex];
    ...
    if (this._mouseDown) {
      ...
      if (this._addingPoint) {
        ...
        if (freq >= 0 && freq < Config.filterFreqRange) {
          ...
          if (this._forSong) {
            sequence.append(
              new ChangeSongFilterAddPoint(
                this._doc,
                this._useFilterSettings,
                point,
                this._useFilterSettings.controlPointCount,
              ),
            );
          } else {
            sequence.append(
        ...
        if (freq >= 0 && freq < Config.filterFreqRange) {
          if (this._forSong) {
            sequence.append(
              new ChangeSongFilterMovePoint(this._doc, point, point.freq, freq, point.gain, gain, this._selectedIndex),
            );
          } else {
            sequence.append(
          ...
          if (this._forSong) {
            sequence.append(
              new ChangeSongFilterAddPoint(this._doc, this._useFilterSettings, point, this._selectedIndex, true),
            );
          } else {
            sequence.append(
              new ChangeFilterAddPoint(
  ...
  private _whenCursorReleased = (event: Event): void => {
    ...
    if (this.container.offsetParent == null) return;
    if (
      this._mouseDown && (this._doc.lastChangeWas(this._dragChange) || this._writingMods) && this._dragChange != null
    ) {
      if (!this._addingPoint && !this._mouseDragging && !this._touchMode) {
        if (this._selectedIndex < this._useFilterSettings.controlPointCount && this._selectedIndex != -1) {
          ...
          if (this._forSong) {
            ...
            const change: ChangeFilterAddPoint = new ChangeFilterAddPoint(
              this._doc,
              this._useFilterSettings,
              point,
              this._selectedIndex,
  ...
  private _updatePath(): void {
    ...
    for (let i: number = 0; i < this._useFilterSettings.controlPointCount; i++) {
      ...
      if (
        ...
        && (this._mouseOver || this._mouseDown) && !this._deletingPoint
      ) {
        this._label.textContent = (i + 1) + ": " + Config.filterTypeNames[point.type]
          + (this._larger ? " @" + prettyNumber(point.getHz()) + "Hz" : "");
      }
  ...
  public undo(): number {
    if (this.selfUndoHistoryPos > 0) {
      ...
      if (
        this.selfUndoSettings[this.selfUndoHistoryPos + 1] != null
        && this.selfUndoSettings[this.selfUndoHistoryPos + 1].startsWith("jmp")
      ) {
  ...
  public swapToSubfilter(oldIndex: number, newIndex: number, useHistory: boolean = false) {
    if (oldIndex != newIndex) {
      ...
      if (useHistory) {
        ...
        this.selfUndoSettings.push(
          "jmp" + oldIndex + "|" + newIndex + ":" + JSON.stringify(this._subFilters[newIndex].toJsonObject()),
        );
        this.selfUndoHistoryPos++;
      }
  ...
  private _getTargetFilterSettings(instrument: Instrument): FilterSettings {
    // TODO: Re-compute default point freqs/gains only when needed
    let targetSettings: FilterSettings = (this._useNoteFilter)
  ...
  public render(activeMods: boolean = false, forceModRender: boolean = false): void {
    ...
    const instrument: Instrument =
      this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()];
    const filterSettings: FilterSettings = this._forSong
      ? this._doc.song.eqFilter
      : (this._useNoteFilter ? instrument.noteFilter : instrument.eqFilter);
    const displayMods: boolean = activeMods && !this._larger
      && (forceModRender || (!this._mouseOver && !this._mouseDragging && !this._mouseDown)) && this._doc.synth.playing;
    if (displayMods) {
    ...
    if (displayMods) {
      this._useFilterSettings = this._forSong
        ? this._getTargetFilterSettingsForSong(this._doc.song)
        : this._getTargetFilterSettings(instrument);
