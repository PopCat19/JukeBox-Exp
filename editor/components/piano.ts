// Piano
//
// Purpose: Renders the piano keyboard display and handles live note preview playback
//
// This module:
// - Draws piano key visualization with active note highlighting
// - Handles mouse and touch input for note preview
...
export class Piano {
  ...
  private readonly _preview: HTMLDivElement = HTML.div({
    style:
      `width: 100%; height: 40px; border: 2px solid ${ColorConfig.primaryText}; position: absolute; box-sizing: border-box; pointer-events: none;`,
  });
  ...
  public static getBassCutoffPitch(doc: SongDocument): number {
    ...
    return octaveOffset * Config.pitchesPerOctave
      + Math.floor(doc.getVisiblePitchCount() / (Config.pitchesPerOctave * 2)) * Config.pitchesPerOctave;
  }
  ...
  constructor(private _doc: SongDocument) {
    ...
    for (let i: number = 0; i < Config.modCount; i++) {
      const firstRowText: SVGTextElement = SVG.text({
        class: "modulator-label",
        "text-anchor": "left",
        fill: ColorConfig.modLabelPrimaryText,
        style:
          "font-weight: bold; align-self: flex-start; transform-origin: center; transform: rotate(-90deg) translate(-19px, 39px); font-size: 11px; font-family: sans-serif;",
      });
      const secondRowText: SVGTextElement = SVG.text({
        class: "modulator-label",
        "text-anchor": "left",
        fill: ColorConfig.modLabelPrimaryText,
        style:
          "font-weight: bold; align-self: flex-end; transform-origin: center; transform: rotate(-90deg) translate(-26px, 42px); font-size: 11px; font-family: sans-serif;",
      });
      const countText: SVGTextElement = SVG.text({
        class: "modulator-inverse-label",
        fill: ColorConfig.modLabelPrimary,
        style:
          "font-weight: bold; align-self: flex-start; transform-origin: center; transform: rotate(-90deg) translate(4px, 13px); font-size: 11px; font-family: sans-serif;",
      });
      const countRect: SVGRectElement = SVG.rect({
        width: "12px",
        height: "9px",
        fill: ColorConfig.indicatorPrimary,
        style: "pointer-events: none; transform: translate(4px, 4px);",
      });

      const firstRowSVG: SVGSVGElement = SVG.svg({
        viewBox: "0 0 16 66",
        width: "16px",
      ...
      const flexRow1: HTMLDivElement = HTML.div({
        style: "display: flex; flex-direction: column; justify-content: space-between; pointer-events: none;",
      }, [
        countSVG,
        firstRowSVG,
      ]);
      const flexRow2: HTMLDivElement = HTML.div({
        style: "display: flex; flex-direction: column-reverse; justify-content: space-between; pointer-events: none;",
      }, [
        secondRowSVG,
      ]);

      const flexContainer: HTMLDivElement = HTML.div({
        style:
          "display: flex; flex-direction: row; justify-content: space-between; padding: 0px; width: 32px; height: 100%; overflow: hidden; pointer-events: none;",
      }, [
        flexRow1,
        flexRow2,
      ]);
  ...
  private _updateCursorPitch(): void {
    const scale: ReadonlyArray<boolean> = this._doc.song.scale == Config.scales.dictionary["Custom"].index
    ...
    if (
      scale[Math.floor(mousePitch) % Config.pitchesPerOctave] || this._doc.song.getChannelIsNoise(this._doc.channel)
    ) {
  ...
  private _documentChanged = (): void => {
    ...
    if (
      (this._renderedScale == this._doc.song.scale && this._doc.song.scale != Config.scales.dictionary["Custom"].index)
      && this._renderedKey == this._doc.song.key && this._renderedDrums == isDrum && this._renderedMod == isMod
    ...
    if (!isDrum && !isMod) {
      if (this._renderedPitchCount != this._pitchCount) {
        ...
        for (let i: number = 0; i < this._pitchCount; i++) {
          const pianoLabel: HTMLDivElement = HTML.div({
            class: "piano-label",
            style:
              "font-weight: bold; -webkit-text-stroke-width: 0; font-size: 11px; font-family: sans-serif; position: absolute; padding-left: 15px; white-space: nowrap;",
          });
      ...
      for (let j: number = 0; j < this._pitchCount; j++) {
        ...
        if (!scale[j % Config.pitchesPerOctave]) {
          ...
          /* @jummbus - Visual distinction for bass notes during live input. Axed for now... maybe needs new colors?
                    filter: hue-shift(60deg) would be ideal but keys are usually grayscale, and filter: is used already anyway for displaying played notes as pressed.
      ...
      for (let j: number = 0; j < Config.modCount; j++) {
        ...
        if (usingMod && status != 0 && status != 3) {
          modCountRect.style.fill =
            ColorConfig.getChannelColor(this._doc.song, instrument.modChannels[Config.modCount - j - 1]).primaryChannel;
        } else {
        ...
        if (this._doc.song.channels[Math.max(0, instrument.modChannels[Config.modCount - j - 1])].name != "") {
          ...
          firstLabel.style.transform = "rotate(-90deg) translate("
            + (-20 - squeeze - Math.round(Math.max(0, (height - 80) / 2))) + "px, 39px) scale(" + scaleFactor + ", 1)";
          ...
          while (scaleFactor == "0.65" && firstLabel.getComputedTextLength() > height + 8) {
            ...
            firstLabel.textContent = firstLabel.textContent.substr(0, firstLabel.textContent.length - offset)
              + firstLabel.textContent.substr(firstLabel.textContent.length - offset + 1);
          }
        } else {
          const height: number = firstLabel.parentElement!.parentElement!.getBoundingClientRect().height;
          firstLabel.style.transform = "rotate(-90deg) translate(" + (-20 - Math.round(Math.max(0, (height - 80) / 2)))
            + "px, 39px) scale(1, 1)";
        }
      }
    }
