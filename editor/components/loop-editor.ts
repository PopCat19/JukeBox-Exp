...
export class LoopEditor {
  ...
  private readonly _barLoop: SVGPathElement = SVG.path({
    fill: "none",
  ...
  private _updateCursorStatus(): void {
    ...
    if (bar >= this._loopAtPointStart && bar <= this._loopAtPointEnd + 1) {
      ...
      bar > this._doc.song.loopStart - 0.25 && bar < this._doc.song.loopStart + this._doc.song.loopLength + 0.25
    ) {
