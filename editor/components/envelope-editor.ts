...
export class EnvelopeEditor {
  ...
  private _onChange = (event: Event): void => {
    ...
    if (targetSelectIndex != -1) {
      ...
      this._doc.record(
        new ChangeSetEnvelopeType(this._doc, envelopeIndex, this._envelopeSelects[envelopeIndex].selectedIndex),
      );
      ...
      this._doc.record(
        new ChangeSetEnvelopeWaveform(this._doc, this._waveformSelects[waveformSelectIndex].value, waveformSelectIndex),
      );
      ...
      startBoxIndex != -1 || endBoxIndex != -1 || startSliderIndex != -1 || endSliderIndex != -1
      || lowerBoundBoxIndex != -1 || upperBoundBoxIndex != -1 || randomStepsBoxIndex != -1 || randomSeedBoxIndex != -1
      || randomStepsSliderIndex != -1 || randomSeedSliderIndex != -1 || LFOStepsBoxIndex != -1
  ...
  private _onInput = (event: Event): void => {
    ...
    if (startBoxIndex != -1) {
      this._lastChange = new ChangeEnvelopePitchStart(
        this._doc,
        parseInt(this.pitchStartBoxes[startBoxIndex].value),
        startBoxIndex,
      );
    } else if (endBoxIndex != -1) {
      this._lastChange = new ChangeEnvelopePitchEnd(
        this._doc,
        parseInt(this.pitchEndBoxes[endBoxIndex].value),
        endBoxIndex,
      );
    } else if (startSliderIndex != -1) {
      this._lastChange = new ChangeEnvelopePitchStart(
        this._doc,
        parseInt(this._pitchStartSliders[startSliderIndex].value),
        startSliderIndex,
      );
    } else if (endSliderIndex != -1) {
      this._lastChange = new ChangeEnvelopePitchEnd(
        this._doc,
        parseInt(this._pitchEndSliders[endSliderIndex].value),
        endSliderIndex,
      );
    ...
    for (let i = index; i < Config.maxEnvelopeCount; i++) {
      if (i >= instrument.envelopeCount) {
        ...
        if (Config.newEnvelopes[instrument.envelopes[i].envelope].name == "pitch") {
          ...
          this._pitchStartSliders[i].max = (instrument.isNoiseInstrument ? Config.drumCount - 1 : Config.maxPitch)
            .toString();
          this.pitchStartBoxes[i].max = (instrument.isNoiseInstrument ? Config.drumCount - 1 : Config.maxPitch)
            .toString();
          this._pitchEndSliders[i].max = (instrument.isNoiseInstrument ? Config.drumCount - 1 : Config.maxPitch)
            .toString();
          this.pitchEndBoxes[i].max = (instrument.isNoiseInstrument ? Config.drumCount - 1 : Config.maxPitch)
            .toString();
          ...
          this._startNoteDisplays[i].textContent = "Start "
            + this._pitchToNote(parseInt(this.pitchStartBoxes[i].value), instrument.isNoiseInstrument) + ": ";
          this._endNoteDisplays[i].textContent = "End "
            + this._pitchToNote(parseInt(this.pitchEndBoxes[i].value), instrument.isNoiseInstrument) + ": ";
          ...
          // update values
          const isRandomTime: boolean = instrument.envelopes[i].waveform == RandomEnvelopeTypes.time
          ...
          this.perEnvelopeSpeedSliders[i].updateValue(
            EnvelopeEditor.convertIndexSpeed(instrument.envelopes[i].perEnvelopeSpeed, "index"),
          );
          if (instrument.envelopes[i].waveform > RandomEnvelopeTypes.length) instrument.envelopes[i].waveform = 0;
          this._randomStepsWrappers[i].style.display = instrument.envelopes[i].waveform == RandomEnvelopeTypes.time
  ...
  private updateSpeedDisplay(envelopeIndex: number) {
    this._perEnvelopeSpeedDisplays[envelopeIndex].textContent = "Spd: x"
      + prettyNumber(
        this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()]
  ...
  public render(): void {
    ...
    for (let envelopeIndex: number = this._rows.length; envelopeIndex < instrument.envelopeCount; envelopeIndex++) {
      ...
      const pitchStartNoteWrapper: HTMLDivElement = HTML.div(
        {
          style:
            "margin-top: 3px; flex:1; display:flex; flex-direction: row; align-items:center; justify-content:right;",
        },
      ...
      const extraLFOSettingsGroup: HTMLDivElement = HTML.div(
        {
          ...
          style:
            "margin-top: 3px; flex:1; display:flex; flex-direction: column; align-items:center; justify-content:right;",
        },
        waveformWrapper,
        LFOStepsWrapper,
      );
      ...
      const perEnvelopeSpeedSlider: Slider = new Slider(
        HTML.input({
          ...
          value: EnvelopeEditor.convertIndexSpeed(instrument.envelopes[envelopeIndex].perEnvelopeSpeed, "index"),
          step: "1",
        }),
        this._doc,
      ...
      const perEnvelopeSpeedDisplay: HTMLSpanElement = HTML.span(
        ...
        "Spd: x"
          + prettyNumber(
            EnvelopeEditor.convertIndexSpeed(perEnvelopeSpeedSlider.getValueBeforeProspectiveChange(), "speed"),
          ),
      );
      ...
      const checkboxWrapper: HTMLDivElement = HTML.div(
        ...
        HTML.span({ class: "tip", onclick: () => this._openPrompt("envelopeInvert") }, "‣ Invert: "),
        invertBox,
        HTML.span(
          { class: "tip", style: "margin-left:4px;", onclick: () => this._openPrompt("discreteEnvelope") },
          "‣ Discrete:",
        ),
        discreteEnvelopeBox,
      );
      ...
      const copyPasteContainer: HTMLDivElement = HTML.div(
        { class: "editor-controls", style: "margin: 0.5em; display: flex; flex-direction:row; align-items:center;" },
        envelopeCopyButton,
        envelopePasteButton,
      );
    ...
    for (let envelopeIndex: number = 0; envelopeIndex < instrument.envelopeCount; envelopeIndex++) {
      this._targetSelects[envelopeIndex].value = String(
        instrument.envelopes[envelopeIndex].target
          + instrument.envelopes[envelopeIndex].index * Config.instrumentAutomationTargets.length,
      );
