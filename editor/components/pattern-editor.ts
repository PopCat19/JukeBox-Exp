...
export class PatternEditor {
    ...
    this._svg = SVG.svg(
      {
        ...
        style: `background-image: url(${
          getLocalStorageItem("customTheme", "")
  ...
  private _getMaxPitch(): number {
    return this._doc.song.getChannelIsMod(this._doc.channel)
      ? Config.modCount - 1
      : (this._doc.song.getChannelIsNoise(this._doc.channel) ? Config.drumCount - 1 : Config.maxPitch);
  }
  ...
  private _validateModDragLabelInput = (event: Event): void => {
    ...
    if (label.innerText != "" && label.innerText != "-") {
      ...
      const xOffset: number = (+(presValue >= 10.0)) + (+(presValue >= 100.0)) + (+(presValue < 0.0))
        + (+(presValue <= -10.0));
  ...
  private _updateCursorStatus(): void {
    ...
    if (
      this._mouseX < 0 || this._mouseX > this._editorWidth || this._mouseY < 0 || this._mouseY > this._editorHeight
      || this._pitchHeight <= 0
    ) return;

    const minDivision: number = this._getMinDivision();
    this._cursor.exactPart = this._mouseX / this._partWidth;
    this._cursor.part = Math.floor(
      Math.max(0, Math.min(this._doc.song.beatsPerBar * Config.partsPerBeat - minDivision, this._cursor.exactPart))
        / minDivision,
    ) * minDivision;
      ...
      if (this._doc.song.getChannelIsMod(this._doc.channel) && !this.editingModLabel) {
        if (this._pattern.notes[this._cursor.curIndex] != null && this._cursor.curNote != null) {
          ...
          const setting: number =
            this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument(this._barOffset)]
              .modulators[mod];

          const presValue: number = this._cursor.curNote.pins[pinIdx].size
          ...
          this._modDragValueLabelLeft = +prettyNumber(
            Math.max(
              Math.min(
                this._editorWidth - 10 - xOffset * 8,
                this._partWidth * (this._cursor.curNote.start + this._cursor.curNote.pins[pinIdx].time) - 4
                  - xOffset * 4,
              ),
              2,
            ),
          ...
          this._modDragValueLabelTop = +prettyNumber(
            this._pitchToPixelHeight(this._cursor.curNote.pitches[0] - this._octaveOffset) - 17
          ...
          this._modDragUpperBound = Config.modulators[setting].convertRealFactor
  ...
  private _cursorIsInSelection(): boolean {
    return this._cursor.valid && this._doc.selection.patternSelectionActive
  ...
  private _cursorAtStartOfSelection(): boolean {
    return this._cursor.valid && this._doc.selection.patternSelectionActive && this._cursor.pitchIndex == -1
  ...
  private _findMousePitch(pixelY: number): number {
    return Math.max(0, Math.min(this._pitchCount - 1, this._pitchCount - (pixelY / this._pitchHeight)))
      + this._octaveOffset;
  }
  ...
  private _snapToPitch(guess: number, min: number, max: number): number {
    ...
    if (
      scale[Math.floor(guess) % Config.pitchesPerOctave] || this._doc.song.getChannelIsNoise(this._doc.channel)
      || this._doc.song.getChannelIsMod(this._doc.channel)
    ) {
  ...
  private _animatePlayhead = (timestamp: number): void => {
    if (
      this._usingTouch && !this.shiftMode && !this._mouseDragging && this._mouseDown
      && performance.now() > this._touchTime + 1000 && this._cursor.valid && this._doc.lastChangeWas(this._dragChange)
    ) {
    ...
    if (
      ...
      && ((this._pattern != null
        && this._doc.song.getPattern(this._doc.channel, Math.floor(this._doc.synth.playhead)) == this._pattern)
      ...
      for (let i = 0; i < noteFlashElements.length; i++) {
        ...
        const noteStart: number = Number(element.getAttribute("note-start"))
          / (this._doc.song.beatsPerBar * Config.partsPerBeat);
        const noteEnd: number = Number(element.getAttribute("note-end"))
          / (this._doc.song.beatsPerBar * Config.partsPerBeat);
    ...
    if (
      this._doc.synth.playing && (this._doc.synth.recording || this._doc.prefs.autoFollow)
  ...
  private _whenMousePressed = (event: MouseEvent): void => {
    ...
    this._mouseX = ((event.clientX || event.pageX) - boundingRect.left) * this._editorWidth
      / (boundingRect.right - boundingRect.left);
    this._mouseY = ((event.clientY || event.pageY) - boundingRect.top) * this._editorHeight
  ...
  private _whenTouchPressed = (event: TouchEvent): void => {
    event.preventDefault();
    const boundingRect: ClientRect = this._svg.getBoundingClientRect();
    this._mouseX = (event.touches[0].clientX - boundingRect.left) * this._editorWidth
      / (boundingRect.right - boundingRect.left);
    this._mouseY = (event.touches[0].clientY - boundingRect.top) * this._editorHeight
      / (boundingRect.bottom - boundingRect.top);
    if (isNaN(this._mouseX)) this._mouseX = 0;
    if (isNaN(this._mouseY)) this._mouseY = 0;
  ...
  public setModSettingsForChange(change: any, songEditor: SongEditor): boolean {
    ...
    function getMatchingInstrumentAndMod(
      ...
      for (let instrumentIndex: number = startIndex; instrumentIndex <= endIndex; instrumentIndex++) {
        ...
        for (let mod: number = 0; mod < Config.modCount; mod++) {
          ...
          if (
            instrument.modulators[mod] == applyToMod && !Config.modulators[instrument.modulators[mod]].forSong
            ...
            if (
              ...
              if (
                ...
                modEnvIndex != undefined
                  && applyToMod == Config.modulators.dictionary["individual envelope speed"].index
    ...
    if (change == null) {
      ...
      this._doc.continuingModRecordingChange != null && this._doc.continuingModRecordingChange.storedChange == null
      ...
      change instanceof ChangeSequence && change.checkFirst() instanceof ChangeSongFilterMovePoint
      && !change.isCommitted()
    ) {
      ...
      change instanceof ChangeSequence && change.checkFirst() instanceof ChangeFilterMovePoint && !change.isCommitted()
    ) {
      // Pushes some pieces of data in each array, to be handled individually down below.
      //   applyToMods:
      ...
      if (toApply) {
        applyValues.push(
          EnvelopeEditor.convertIndexSpeed(instrument.envelopes[envelopeIndex].perEnvelopeSpeed, "index")
      ...
      if (slider != null) {
        instrument.envelopes[envelopeIndex].perEnvelopeSpeed = EnvelopeEditor.convertIndexSpeed(
    ...
    for (let applyIndex: number = 0; applyIndex < applyToMods.length; applyIndex++) {
        ...
        for (
          ...
          if (pattern != null) {
            ...
            for (let instrumentIndex: number = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
              for (let mod: number = 0; mod < Config.modCount; mod++) {
                if (
                  channel.instruments[instrumentIndex].modulators[mod] == Config.modulators.dictionary["none"].index
                ) {
                ...
                if (Config.modulators[applyToMods[applyIndex]].forSong) {
                  ...
                  if (this._doc.song.channels[this._doc.channel].instruments.length > 1) {
                    ...
                    if (!this.controlMode || !this.shiftMode) {
                      instrument.modInstruments[mod] = this._doc.song.channels[this._doc.channel].instruments.length
                        + 1;
      ...
      for (let i: number = 0; i < usedPatterns.length; i++) {
        ...
        if (usedInstrument.modChannels[usedModIndices[i]] >= -1) {
          ...
          if (Config.modulators[applyToMods[applyIndex]].forSong) {
            ...
            if (
              ...
              for (
                let k: number = 0;
                k < this._doc.synth.song!.channels[usedInstrument.modChannels[usedModIndices[i]]].instruments.length;
                k++
              ) {
              ...
              if (this._doc.synth.song!.getPattern(usedInstrument.modChannels[usedModIndices[i]], currentBar) != null) {
                usedNewInstrumentIndices =
                  this._doc.synth.song!.getPattern(usedInstrument.modChannels[usedModIndices[i]], currentBar)!
                    .instruments;
              }
            } else {
        ...
        for (let checkIndex: number = 0; checkIndex < usedPatterns[i].notes.length; checkIndex++) {
          ...
          if (
            note.pins.length < 2 || note.pins[0].time > 0 || note.start == note.end
  ...
  private _whenCursorPressed(): void {
    ...
    if (
      ...
      if (this._cursorAtStartOfSelection()) {
        ...
        if (
          (this._doc.selection.patternSelectionActive && this._cursor.pitchIndex == -1) || this._cursorIsInSelection()
        ) {
          sequence.append(new ChangePatternSelection(this._doc, 0, 0));
        } else {
          if (this._cursor.curNote != null) {
            sequence.append(
              new ChangePatternSelection(this._doc, this._cursor.curNote.start, this._cursor.curNote.end),
            );
          } else {
    ...
    if (this._mouseDragging && this._mouseDown && this._cursor.valid && continuousState) {
      ...
      if (this._draggingStartOfSelection) {
        ...
        if (this._mouseDragging && pattern != null) {
          ...
          const scale = this._doc.song.scale == Config.scales.dictionary["Custom"].index
            ? this._doc.song.scaleCustom
            : Config.scales[this._doc.song.scale].flags;
          ...
          const draggedParts: number = Math.round((this._mouseX - this._mouseXStart) / (this._partWidth * minDivision))
            * minDivision;
          const draggedTranspose: number = Math.round(
            (this._mouseYStart - this._mouseY) / (this._pitchHeight * pitchRatio),
          );
          sequence.append(
            new ChangeDragSelectedNotes(this._doc, this._doc.channel, pattern, draggedParts, draggedTranspose),
          );
        }
        ...
        if (this._cursor.curNote == null) {
          ...
          if (start < end) {
            ...
            sequence.append(
              new ChangeNoteTruncate(this._doc, pattern, start, end, new Note(this._cursor.pitch, 0, 0, 0)),
            );
            let i: number;
          ...
          if (
            this._pattern != null && this._doc.song.getChannelIsMod(this._doc.channel) && this._interactive
          ...
          let shiftedTime: number = Math.round((this._cursor.curNote.start + shiftedPin.time + shift) / minDivision)
            * minDivision;
          ...
          if (
            ...
            sequence.append(
              new ChangeNoteAdded(this._doc, this._pattern, this._cursor.curNote, this._cursor.curIndex, true),
            );
            ...
            this._dragPitch = this._cursor.curNote.pitches[this._cursor.pitchIndex == -1 ? 0 : this._cursor.pitchIndex]
          ...
          const bendPart: number = Math.max(
          ...
          const dragCounts: number = Math.min(Math.abs(this._mouseYStart - this._mouseY) / dragFactorSlow, 8)
          ...
          for (let i: number = 1; i < this._cursor.curNote.pins.length; i++) {
            ...
            if (
              !this.controlMode && !this._doc.prefs.alwaysFineNoteVol
              && !this._doc.song.getChannelIsMod(this._doc.channel)
            ) {
            ...
            bendInterval = this._snapToPitch(
              ...
              if (
                ...
                for (const note of this._pattern!.notes) {
                  if (
                    ...
                    sequence.append(
                      new ChangeSizeBend(this._doc, note, note.pins[0].time, bendSize, bendInterval, this.shiftMode),
                    );
                  }
                }
              } else {
          ...
          sequence.append(
            new ChangeSizeBend(this._doc, this._cursor.curNote, bendPart, bendSize, bendInterval, this.shiftMode),
          );
          this._copyPins(this._cursor.curNote);
        } else {
          sequence.append(new ChangePatternSelection(this._doc, 0, 0));
          ...
          if (bendEnd < this._cursor.curNote.start) {
            sequence.append(
              new ChangeNoteTruncate(this._doc, this._pattern, bendEnd, this._cursor.curNote.end, this._cursor.curNote),
            );
          }
          ...
          if (!this._doc.song.getChannelIsMod(this._doc.channel)) {
            ...
            sequence.append(
              new ChangePitchBend(this._doc, this._cursor.curNote, bendStart, bendEnd, bendTo, this._cursor.pitchIndex),
            );
  ...
  private _whenCursorReleased = (event: Event | null): void => {
    ...
    if (this._mouseDown && continuousState && this._dragChange != null) {
      if (this._draggingSelectionContents) {
        ...
        this._mouseDragging || this._cursor.curNote == null || !this._dragChange.isNoop()
        || this._draggingStartOfSelection || this._draggingEndOfSelection || this._draggingSelectionContents
        || this._shiftHeld
      ) {
        this._doc.record(this._dragChange);
        this._dragChange = null;
        ...
        if (this._cursor.pitchIndex == -1) {
          if (this._cursor.curNote.pitches.length == Config.maxChordSize) {
            sequence.append(
              new ChangePitchAdded(this._doc, this._cursor.curNote, this._cursor.curNote.pitches[0], 0, true),
            );
          }
          sequence.append(
  ...
  private _updatePreview(): void {
    if (this._usingTouch) {
      if (
        !this._mouseDown || !this._cursor.valid || !this._mouseDragging || !this._dragVisible || this._shiftHeld
        || this._draggingStartOfSelection || this._draggingEndOfSelection || this._draggingSelectionContents
      ) {
    ...
    if (this._interactive) {
      ...
      if (anyNoteRangeIsEnabled && anyNoteRangeIsDifferent) {
        ...
        for (let i: number = 1; i < this._maximumNoteRanges; i++) {
          ...
          const shouldMerge: boolean = noteRangesOverlap(range, noteRangeToMergeWith)
      ...
      const changed: boolean = forceRedraw
        ...
        if (anyNoteRangeIsEnabled) {
          ...
          if (allNoteRangesAreEmpty) {
            ...
            const topY1: number = Math.min(
              ...
              Math.max(
                0,
                this._pitchToPixelHeight(this._renderedNoteRangeHighestNoteLimit - this._octaveOffset)
                  - this._pitchHeight / 2,
              ),
            );

            const topIsVisible: boolean = this._renderedNoteRangeHighestNoteLimit != -1
              && topY0 < topY1;
            ...
            const bottomY0: number = Math.min(
              ...
              Math.max(
                0,
                this._pitchToPixelHeight(this._renderedNoteRangeLowestNoteLimit - this._octaveOffset)
                  + this._pitchHeight / 2,
              ),
            );
            const bottomY1: number = height;

            const bottomIsVisible: boolean = this._renderedNoteRangeLowestNoteLimit != -1
              && bottomY0 < bottomY1;
  ...
  public render(): void {
    ...
    this._octaveOffset = (this._doc.channel >= this._doc.song.pitchChannelCount)
      ? 0
  ...
  private _redrawNotePatterns(): void {
    ...
    if (this._doc.prefs.showChannels) {
      if (!this._doc.song.getChannelIsMod(this._doc.channel)) {
        ...
        for (
          let channel: number = this._doc.song.pitchChannelCount + this._doc.song.noiseChannelCount - 1;
          channel >= 0;
          channel--
        ) {
    ...
    if (this._pattern != null) {
      ...
      const displayNumberedChords: boolean = chord.customInterval || chord.arpeggiates || chord.strumParts > 0
        || transition.slides || chord.name == "monophonic";
      ...
      for (const note of this._pattern.notes) {
        ...
        for (let i: number = 0; i < note.pitches.length; i++) {
          ...
          if (note.continuesLastPattern) {
            ...
            arrowPath = "M " + prettyNumber(this._partWidth * note.start + indicatorOffset) + " "
              + prettyNumber(this._pitchToPixelHeight(pitch - this._octaveOffset) - 0.1 * arrowHeight);
            arrowPath += "L " + prettyNumber(this._partWidth * note.start + indicatorOffset) + " "
              + prettyNumber(this._pitchToPixelHeight(pitch - this._octaveOffset) + 0.1 * arrowHeight);
            arrowPath += "L " + prettyNumber(this._partWidth * note.start + indicatorOffset + 4) + " "
              + prettyNumber(this._pitchToPixelHeight(pitch - this._octaveOffset) + 0.1 * arrowHeight);
            arrowPath += "L " + prettyNumber(this._partWidth * note.start + indicatorOffset + 4) + " "
              + prettyNumber(this._pitchToPixelHeight(pitch - this._octaveOffset) + 0.3 * arrowHeight);
            arrowPath += "L " + prettyNumber(this._partWidth * note.start + indicatorOffset + 12) + " "
        ...
        if (
          this._doc.song.getChannelIsMod(this._doc.channel) && this._mouseDragging && !this._mouseHorizontal
          ...
          this._modDragValueLabelLeft = +prettyNumber(
            Math.max(
              Math.min(this._editorWidth - 10 - xOffset * 8, this._partWidth * this._dragTime - 4 - xOffset * 4),
              2,
            ),
          );
          this._modDragValueLabelTop = +prettyNumber(
            this._pitchToPixelHeight(note.pitches[0] - this._octaveOffset) - 17
              - (this._pitchHeight - this._pitchBorder) / 2,
          );
  ...
  private _drawNote(
    ...
    let pathString: string = "M " + prettyNumber(this._partWidth * (start + nextPin.time) + endOffset) + " "
