...
import { FilterControlPoint, FilterSettings, HarmonicsWave, Instrument, SpectrumWave } from "../../synth";
import {
  Algorithm,
  Config,
...
export class ChangeCustomWave extends Change {
  constructor(doc: SongDocument, newArray: Float32Array) {
    ...
    const oldArray: Float32Array =
      doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()].customChipWave;
  ...
  constructor(doc: SongDocument, newArray: number[][], carry: number, mode: string) {
    ...
    if (mode == "algorithm") {
      const oldArray: number[][] =
        doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()].customAlgorithm.modulatedBy;
      const oldCarriercount: number =
        doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()].customAlgorithm.carrierCount;
      let comparisonResult: boolean = true;
      ...
      const oldArray: number[][] =
        doc.song.channels[doc.channel].instruments[doc.getCurrentInstrument()].customFeedbackType.indices;
      let comparisonResult: boolean = true;
      for (let i: number = 0; i < oldArray.length; i++) {
        if (oldArray[i].length != newArray[i].length) {
          comparisonResult = false;
    ...
    if (oldValue != newValue) {
      ...
      if (preset != null) {
        if (preset.customType != undefined) {
          ...
          if (
            !Config.instrumentTypeHasSpecialInterval[instrument.type] && Config.chords[instrument.chord].customInterval
          ) {
            instrument.chord = 0;
          }
  ...
  constructor(doc: SongDocument, usesCurrentInstrumentType: boolean) {
    ...
    if (isNoise) {
      ...
      if (Math.random() < 0.1) {
        ...
        instrument.transition = Config.transitions.dictionary[
          selectWeightedRandom([
      ...
      if (Math.random() < 0.2) {
        ...
        instrument.chord = Config.chords.dictionary[
          selectWeightedRandom([
            { item: "strum", weight: 2 },
      ...
      if (Math.random() < 0.1) {
        ...
        if (instrument.pitchShift != Config.pitchShiftCenter) {
          ...
          instrument.addEnvelope(
            ...
            0,
            -1,
            selectWeightedRandom([{ item: false, weight: 8 }, { item: true, weight: 1 }]),
      ...
      if (Math.random() < 0.1) {
        ...
        instrument.vibrato = Config.vibratos.dictionary[
          selectWeightedRandom([
            { item: "light", weight: 2 },
      ...
      if (type == InstrumentType.noise || type == InstrumentType.spectrum) {
        instrument.unison = Config.unisons.dictionary[
          selectWeightedRandom([
      ...
      instrument.fadeOut = selectCurvedDistribution(0, Config.fadeOutTicks.length - 1, Config.fadeOutNeutral, 2);
      if (
        type == InstrumentType.chip || type == InstrumentType.harmonics || type == InstrumentType.pickedString
        || type == InstrumentType.customChipWave || type == InstrumentType.pwm || type == InstrumentType.spectrum
      ) { // TODO: add noise
        instrument.unison = Config.unisons.dictionary[
          selectWeightedRandom([
            { item: "none", weight: 100 },
      ...
      if (Math.random() < 0.1) {
        ...
        if (Math.random() < 0.3) {
          ...
          instrument.addEnvelope(
            ...
            selectWeightedRandom([{ item: LFOEnvelopeTypes.sine, weight: 2 }, {
      ...
      if (Math.random() < 0.1) {
        ...
        if (Math.random() < 0.3) {
          instrument.addEnvelope(
            ...
            selectCurvedDistribution(2, 16, 2, 6),
            selectCurvedDistribution(1, 63, 32, 31),
            selectWeightedRandom([{ item: LFOEnvelopeTypes.sine, weight: 3 }, {
              item: LFOEnvelopeTypes.triangle,
              weight: 1,
            }]),
          );
        ...
        if (instrument.echoSustain != 0 || instrument.echoDelay != 0) {
          ...
          if (Math.random() < 0.04) {
            ...
            instrument.addEnvelope(
              ...
              selectCurvedDistribution(2, 16, 2, 6),
              selectCurvedDistribution(1, 63, 32, 31),
              selectWeightedRandom([{ item: LFOEnvelopeTypes.sine, weight: 8 }, {
                item: LFOEnvelopeTypes.triangle,
                weight: 3,
              }]),
            );
      ...
      if (Math.random() < 0.2) {
        ...
        instrument.addEnvelope(
          ...
          selectCurvedDistribution(2, 16, 2, 6),
          selectCurvedDistribution(1, 63, 32, 31),
          selectWeightedRandom([{ item: RandomEnvelopeTypes.time, weight: 8 }, {
        ...
        case InstrumentType.fm:
          {
            ...
            const algorithm: Algorithm = type == InstrumentType.fm
            ...
            for (let i: number = 0; i < algorithm.carrierCount; i++) {
              ...
              instrument.operators[i].waveform = Config.operatorWaves.dictionary[
                selectWeightedRandom([
                  { item: "sine", weight: 10 },
