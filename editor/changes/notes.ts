...
export class ChangeEnsurePatternExists extends UndoableChange {
  ...
  constructor(doc: SongDocument, channelIndex: number, bar: number) {
    ...
    if (firstEmptyUnusedIndex != null) {
      ...
      this._oldPatternInstruments = song.channels[channelIndex].patterns[firstEmptyUnusedIndex - 1].instruments
        .concat();
...
export class ChangeRhythmNote extends ChangePins {
  constructor(doc: SongDocument | null, note: Note, changeRhythm: (oldTime: number) => number) {
    ...
    for (const oldPin of this._oldPins) {
      this._newPins.push(
        makeNotePin(oldPin.interval, changeRhythm(oldPin.time + this._oldStart) - this._oldStart, oldPin.size),
      );
    }
...
export class ChangeDetectKey extends ChangeGroup {
  constructor(doc: SongDocument) {
          ...
          for (const note of pattern.notes) {
            ...
            for (let pinIndex: number = 1; pinIndex < note.pins.length; pinIndex++) {
              ...
              if (prevPin.interval == nextPin.interval) {
                ...
                weight += Math.max(
                  0,
                  Math.min(Config.partsPerBeat, nextPin.time + note.start) - (prevPin.time + note.start),
                );
    ...
    for (let key: number = 0; key < 12; key++) {
      ...
      const keyWeight: number = keyWeights[key]
        * (3 * keyWeights[(key + 7) % 12] + keyWeights[(key + 4) % 12] + keyWeights[(key + 3) % 12]);
  ...
  constructor(
    ...
    while (i < pattern.notes.length) {
      ...
      if (note == skipNote && skipNote != null) {
        ...
        if (
          !doc.song.getChannelIsMod(doc.channel) || force
  ...
  constructor(
    ...
    for (let channelIndex: number = channelStart; channelIndex < channelStart + channelHeight; channelIndex++) {
      ...
      for (let bar: number = barStart; bar < barStart + barWidth; bar++) {
        ...
        this.append(
          new ChangePatternNumbers(doc, reusablePatterns[String(currentPatternIndex)], bar, channelIndex, 1, 1),
        );
      }
    }
  }
