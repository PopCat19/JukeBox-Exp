...
export function patternsContainSameInstruments(pattern1Instruments: number[], pattern2Instruments: number[]): boolean {
  const pattern2Has1Instruments: boolean = pattern1Instruments.every(instrument =>
    pattern2Instruments.indexOf(instrument) != -1
  );
  const pattern1Has2Instruments: boolean = pattern2Instruments.every(instrument =>
    pattern1Instruments.indexOf(instrument) != -1
  );
...
export function generateScaleMap(
  ...
  const newScaleFlags: ReadonlyArray<boolean> = newScaleValue == Config.scales["dictionary"]["Custom"].index
    ? customScaleFlags
  ...
  if (roundedWaveType == 1 || roundedWaveType == 3) {
    ...
    for (let i = 0; i < waveLength; i++) {
      randomRoundWave[i] = clamp(
        ...
        24 + 1,
        Math.round(
          mod(randomNumber3 + ((Math.sin((i + randomNumber3) / randomNumber2) * 24) + i * randomNumber1), 48) - 24,
        ),
      );
    }
                ...
                2
                  * Math.floor(
                    Math.sin((i / randomNumber2) * randomNumber1 + randomNumber3)
                      * Math.cos((i * randomNumber2) * (randomNumber1 / 2)) * 24,
                  ),
              ) - randomNumber4 * 24,
        ),
      );
...
export function randomPulses(wave: Float32Array): void {
  ...
  for (let i = 0; i < waveLength; i++) {
    ...
    randomPulse[i] = clamp(
      ...
      24 + 1,
      Math.round(
        mod(24 * (sigma(i, (i) => randomNumber1, Math.round(randomNumber2 / randomNumber3))), 24.0000000000001),
      ),
    );
  }
  for (let i = 0; i < waveLength; i++) {
  ...
  if (chipType == 1) {
    ...
    for (let i = 0; i < waveLength; i++) {
      randomChip[i] = clamp(
        ...
        24 + 1,
        mod(
          Math.round(mod(sigma(i / randomNumber1, (i) => (randomNumber1 * randomNumber3), 0), 25 + randomNumber2) * 24),
          48,
        ) - 24,
      );
    }
...
export function comparePatternNotes(a: Note[], b: Note[]): boolean {
  ...
  for (let noteIndex: number = 0; noteIndex < a.length; noteIndex++) {
    ...
    if (
      newNote.start != oldNote.start || newNote.end != oldNote.end || newNote.pitches.length != oldNote.pitches.length
