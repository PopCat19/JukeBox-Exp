...
export class ExportPrompt extends BasePrompt {
	...
	private static readonly midiChipInstruments: number[] = [0x4a, 0x47, 0x50, 0x46, 0x44, 0x51, 0x51, 0x51, 0x51];
	...
	private _exportToMidi(): void {
		...
		for (const track of tracks) {
			...
			if (track.isMeta) {
				...
				let loopTime = 0;
				if (this._enableIntro.checked) loopTime += midiTicksPerBeat * song.beatsPerBar * song.loopStart;
				writeTime(loopTime);
				writer.writeUint8(MidiEventType.meta);
				writer.writeMidi7Bits(MidiMetaEventMessage.marker);
				writer.writeMidiAscii("Loop Start");
				for (let i = 0; i < parseInt(this._loopDropDown.value); i++) {
					loopTime += midiTicksPerBeat * song.beatsPerBar * song.loopLength;
					writeTime(loopTime);
					writer.writeUint8(MidiEventType.meta);
					writer.writeMidi7Bits(MidiMetaEventMessage.marker);
					writer.writeMidiAscii(i < Number(this._loopDropDown.value) - 1 ? "Loop Repeat" : "Loop End");
				}
				barStartTime = loopTime;
				if (this._enableOutro.checked) barStartTime += midiTicksPerBeat * song.beatsPerBar * (song.barCount - song.loopStart - song.loopLength);
			} else {
				writeTime(0);
				writer.writeUint8(MidiEventType.meta);
