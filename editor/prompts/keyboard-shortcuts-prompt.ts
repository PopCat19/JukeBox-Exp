...
const shortcutCategories: ShortcutCategory[] = [
  {
    ...
    entries: [
      ...
      {
        key: "B",
        desc: "Toggle loop / play from selection",
        detail: "Toggles looping. Hold Shift while pressing B to play from the current selection instead.",
      },
  ...
  },
  {
    ...
    entries: [
      ...
      { key: "C", desc: "Copy" },
      {
        key: "C",
        mods: "Shift",
        desc: "Copy instrument",
        detail: "Copies the current instrument settings to clipboard, separate from pattern copy.",
      },
      { key: "V", desc: "Paste" },
      {
        key: "V",
        mods: "Shift",
        desc: "Paste instrument",
        detail: "Pastes instrument settings from clipboard without changing patterns.",
      },
      {
        key: "V",
        mods: "Ctrl+Shift",
        desc: "Paste numbers only",
        detail: "Pastes only the note digits from clipboard, preserving existing pattern structure.",
      },
      ...
      { key: "A", mods: "Shift", desc: "Select channel" },
      {
        ...
        detail: "Duplicates the patterns in the selected area into an empty pattern slot.",
      },
      {
        key: "N",
        desc: "New pattern (first unused)",
        detail: "Assigns the first available unused pattern to the current bar.",
      },
      {
        key: "N",
        mods: "Ctrl",
        desc: "New pattern (next empty)",
        detail: "Creates the next empty pattern and assigns it to the current bar.",
      },
      {
        key: "0\u20139",
        desc: "Enter digit for selection",
        detail: "Appends a digit to the current selection for fast note/pattern number entry.",
      },
      {
        key: "Delete",
        desc: "Clear selection digits",
        detail: "Clears the entered digits without deleting the note itself.",
      },
    ],
  },
  {
    ...
    entries: [
      {
        key: "Enter",
        desc: "Insert bar",
        detail: "Inserts a bar after the current one. Hold Shift to insert before (shifting current bar right).",
      },
      {
        key: "Enter",
        mods: "Shift",
        desc: "Insert bar (shift left)",
        detail: "Inserts a bar and shifts the current bar to the right.",
      },
      ...
      { key: "Enter", mods: "Alt", desc: "Add instrument" },
      {
        key: "Backspace",
        desc: "Delete bars",
        detail: "Deletes the current bar. Hold Ctrl to delete the current channel instead.",
      },
  ...
  },
  {
    ...
    entries: [
      { key: "M", desc: "Mute channel" },
      {
        key: "S",
        desc: "Solo channel",
        detail: "Solos the current channel, muting all others. Press again to unmute.",
      },
      {
        key: "S",
        mods: "Shift",
        desc: "Exclude channel",
        detail: "Excludes the current channel from playback without affecting other channels' mute state.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    entries: [
      ...
      { key: "R", desc: "Random instrument preset" },
      {
        key: "R",
        mods: "Ctrl+Shift",
        desc: "Random generated instrument",
        detail: "Generates a completely random instrument from scratch, not from presets.",
      },
      ...
      {
        key: "W",
        desc: "Move notes sideways",
        detail: "Shifts all notes in the selection left or right by a specified number of columns.",
      },
  ...
  },
  {
    ...
    entries: [
      ...
      { key: "`", mods: "Shift", desc: "New song" },
      {
        key: "`",
        desc: "Song recovery",
        detail: "Opens the song recovery dialog to restore auto-saved or previously edited songs.",
      },
  ...
  private _renderList(filter: string): void {
    ...
    for (const category of shortcutCategories) {
      ...
      for (const entry of filtered) {
        ...
        const rowStyle = `display: flex; align-items: baseline; gap: 12px; padding: 3px 8px; background: ${bgColor};${
          entry.detail ? " cursor: help;" : ""
        }`;
        const row = div(
          {
            style: rowStyle,
          ...
          span({
            style:
              "font-family: monospace; font-size: 12px; color: var(--secondary-text); white-space: nowrap; min-width: 120px; text-align: right;",
          }, keyCombo),
          span(
            { style: "font-size: 12px;" },
            entry.desc,
            entry.detail
              ? span({ style: "color: var(--secondary-text); font-size: 10px; margin-left: 4px;" }, "?")
              : "",
          ),
        );
