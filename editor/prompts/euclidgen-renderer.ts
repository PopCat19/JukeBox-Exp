...
export function renderClock(ctx: EuclidgenRendererContext): void {
	...
	const color: string = ColorConfig.getChannelColor(
		ctx.song,
		ctx.sequences[ctx.sequenceIndex].channel,
	).primaryNote;
	...
	const clockPointRadius: number = Math.max(
		ctx.clockPointMinRadius,
		Math.min(ctx.clockPointMaxRadius, ctx.clockWidth / steps),
	);
	for (let step: number = 0; step < steps; step++) {
		...
		const x: number =
				...
				Math.cos(angle) *
					(ctx.clockWidth / 2 -
						ctx.clockPointMaxRadius -
						ctx.clockWidth / ctx.maxSequences),
			y: number =
				centerY +
				Math.sin(angle) *
					(ctx.clockWidth / 2 -
...
export function renderBarPreview(ctx: EuclidgenRendererContext): void {
	const beatsPerBar: number =
		ctx.sequences[ctx.sequenceIndex].steps > 0 ? ctx.sequences[ctx.sequenceIndex].steps : 4;
	...
	const firstStep: number = Math.floor(
		(beatsPerBar * (ctx.barPreviewBarIndex - ctx.startBar)) / stepSize,
	);
	const lastStep: number = Math.ceil(
		(beatsPerBar * (ctx.barPreviewBarIndex - ctx.startBar + 1)) / stepSize,
	);
...
export function renderSequenceButtons(ctx: EuclidgenRendererContext): void {
	...
	while (ctx.sequenceButtons.length < ctx.sequences.length) {
		const sequenceButton: HTMLButtonElement = selectorButton(
			`${ctx.sequenceButtons.length + 1}`,
		);
		ctx.sequenceButtons.push(sequenceButton);
		container.insertBefore(sequenceButton, ctx.sequenceRemoveButton);
	}
	for (let i: number = ctx.renderedSequenceCount; i < ctx.sequences.length; i++)
		ctx.sequenceButtons[i].style.display = "";
	for (let i: number = ctx.sequences.length; i < ctx.renderedSequenceCount; i++)
		ctx.sequenceButtons[i].style.display = "none";
	ctx.renderedSequenceCount = ctx.sequences.length;
	while (ctx.sequenceButtons.length > ctx.maxSequences)
		container.removeChild(ctx.sequenceButtons.pop()!);
	...
	ctx.sequenceRemoveButton.classList.toggle(
		"last-button",
		ctx.sequences.length >= ctx.maxSequences,
	);
	if (ctx.highlightedSequenceIndex !== ctx.sequenceIndex) {
		if (ctx.sequenceButtons[ctx.highlightedSequenceIndex])
			ctx.sequenceButtons[ctx.highlightedSequenceIndex].classList.remove(
				"selected-instrument",
			);
	...
	for (let s: number = 0; s < ctx.sequences.length; s++)
		ctx.sequenceButtons[s].style.color =
			s === ctx.highlightedSequenceIndex ? "" : ColorConfig.primaryText;
	...
	ctx.sequenceButtonContainer.style.setProperty(
