...
export class CustomFilterPrompt extends BasePrompt {
  ...
  private readonly _filterCoordinateText: HTMLDivElement = div({
    style: "text-align: left; margin-bottom: 0px; font-size: x-small; height: 1.3em; color: "
  ...
  private _copyFilterSettings = (): void => {
    const filterCopy: any = this.forSong
      ...
      : this._useNoteFilter
      ? this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].noteFilter
        .toJsonObject()
      : this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].eqFilter
        .toJsonObject();
