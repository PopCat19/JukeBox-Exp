...
export class EuclidgenRhythmPrompt extends BasePrompt {
      ...
      div(
        ...
        div(
          ...
          div(
            {
              style:
                "display: flex; flex-direction: row; align-items: center; height: 3em; justify-content: flex-end; margin-top: 0.5em;",
            },
        ...
        div(
          ...
          div(
            {
              style:
                "display: flex; flex-direction: row; align-items: center; height: 3em; justify-content: flex-end; margin-bottom: 1em;",
            },
        ...
        if (rawSequences != null && Array.isArray(rawSequences)) {
          ...
          for (const rawSequence of rawSequences) {
            parsedSequences.push({
              steps: Math.max(
                this._minSteps,
                Math.min(this._maxSteps, rawSequence["steps"] ?? this._doc.song.beatsPerBar),
              ),
  ...
  protected override _saveChanges(): void {
    ...
    for (const [channelIndex, resultingChannel] of allNewNotesByChannel.entries()) {
      for (let resultingBarIndex: number = 0; resultingBarIndex < resultingChannel.length; resultingBarIndex++) {
        ...
        interface MergeableEvent { noteType: "old" | "new"; eventType: "start" | "end"; part: number; note: Note; }
        ...
        interface MergeableEventGroup { part: number; events: MergeableEvent[]; }
        ...
        interface MergeableNote { noteType: "old" | "new"; note: Note; }
          ...
          if (heldNotes.length === 0) {
            ...
            for (const mergeableNote of heldNotes) {
              ...
              if (mergeableNote.noteType === "new") {
                if (theNewNote == null || mergeableNote.note.start > theNewNote.start || mergeableNote.note.end < theNewNote.end) theNewNote = mergeableNote.note;
            ...
            if (theNewNote != null) {
              ...
              if (firstVisibleOldPinIndex !== -1) {
                ...
                if (firstNewPin.time !== 0) {
                  ...
                  const ratio: number = (mergedStartRelativeToOldStart - leftAdjacentOldPin.time) / (firstNewPin.time + (mergedStartRelativeToOldStart - leftAdjacentOldPin.time));
                ...
                if (lastNewPin.time !== mergedNoteLength) {
                  ...
                  const ratio: number = (mergedEndRelativeToOldStart - (lastNewPin.time + mergedStartRelativeToOldStart)) / ((rightAdjacentOldPin.time - mergedEndRelativeToOldStart) + (mergedEndRelativeToOldStart - (lastNewPin.time + mergedStartRelativeToOldStart)));
                  newPins.push(makeNotePin(0, mergedNoteLength, Math.round(lastNewPin.size + (rightAdjacentOldPin.size - lastNewPin.size) * ratio)));
                }
              } else {
                const leftAdjacentOldPin: NotePin = theOldNote.pins[leftAdjacentOldPinIndex];
                const rightAdjacentOldPin: NotePin = theOldNote.pins[rightAdjacentOldPinIndex];
                const lineLength: number = rightAdjacentOldPin.time - leftAdjacentOldPin.time;
                newPins.push(makeNotePin(0, 0, Math.round(leftAdjacentOldPin.size + (rightAdjacentOldPin.size - leftAdjacentOldPin.size) * ((mergedStartRelativeToOldStart - leftAdjacentOldPin.time) / lineLength))));
                newPins.push(makeNotePin(0, mergedNoteLength, Math.round(leftAdjacentOldPin.size + (rightAdjacentOldPin.size - leftAdjacentOldPin.size) * ((mergedEndRelativeToOldStart - leftAdjacentOldPin.time) / lineLength))));
              }
  ...
  private _initialRender = (): void => {
    ...
    for (let beat: number = 0; beat < beatsPerBar; beat++) {
      container.appendChild(SVG.rect({ x: beat * beatWidth + padding, y: padding, width: beatWidth - padding * 2, height: beatHeight - padding * 2, style: `fill: ${color};` }));
    }
    this._refreshSequenceWidgets();
    this._reconfigurePitchStepper();
    this._reconfigurePulsesStepper();
  ...
  private _renderSequenceButtons = (): void => {
    ...
    if (this._highlightedSequenceIndex != this._sequenceIndex) {
      if (this._sequenceButtons[this._highlightedSequenceIndex]) this._sequenceButtons[this._highlightedSequenceIndex].classList.remove("selected-instrument");
    ...
    for (let s: number = 0; s < this._sequences.length; s++) this._sequenceButtons[s].style.color = (s === this._highlightedSequenceIndex) ? "" : ColorConfig.primaryText;
  ...
  private _renderClock = (): void => {
    ...
    const centerX: number = this._clockWidth / 2, centerY: number = this._clockHeight / 2;
    ...
    for (let step: number = 0; step < steps; step++) {
      ...
      const x: number = centerX + Math.cos(angle) * this._clockRadius, y: number = centerY + Math.sin(angle) * this._clockRadius;
      const clockPoint: SVGCircleElement = SVG.circle({ cx: x, cy: y, r: clockPointRadius, style: `stroke: ${color}; stroke-width: 0.5; fill: ${(generatedSequence.length > 0 && generatedSequence[step % steps] === on) ? color : ColorConfig.editorBackground}` });
      container.appendChild(clockPoint);
    }
  };

  private _renderBarPreview = (): void => {
    const beatsPerBar: number = this._doc.song.beatsPerBar, partsPerBeat: number = Config.partsPerBeat, partsPerBar: number = partsPerBeat * beatsPerBar;
    const sequence: Sequence = this._sequences[this._sequenceIndex], steps: number = sequence.steps, stepSize: number = sequence.stepSizeNumerator / sequence.stepSizeDenominator;
    const generatedSequence: number[] = this._generatedSequences[this._sequenceIndex], on: number = sequence.invert ? 0 : 1;
    ...
    const beatWidth: number = this._barPreviewWidth / beatsPerBar, partWidth: number = beatWidth / partsPerBeat, padding: number = 0.2, y: number = padding, h: number = this._barPreviewHeight - padding * 2;
    ...
    for (let step: number = firstStep; step < lastStep; step++) {
      const rawStart: number = Math.floor(step * partsPerBeat * stepSize) - partOffset, rawEnd: number = Math.floor((step + 1) * partsPerBeat * stepSize) - partOffset;
      const stepStart: number = Math.max(0, Math.min(partsPerBar, rawStart)), stepEnd: number = Math.max(0, Math.min(partsPerBar, rawEnd));
      const x: number = padding + stepStart * partWidth, w: number = (stepEnd - stepStart) * partWidth - padding * 2;
      if (generatedSequence.length > 0 && generatedSequence[step % steps] === on) {
        if (sequence.generateFadingNotes) {
          ...
          const startSize: number = (Math.max(0, Math.min(1, 1 - (stepStart - rawStart) / (rawEnd - rawStart)))), endSize: number = (Math.max(0, Math.min(1, 1 - (stepEnd - rawStart) / (rawEnd - rawStart))));
          container.appendChild(SVG.path({ d: `M ${x} ${y + (h / 2) * (1 - startSize)} L ${x + w} ${y + (h / 2) * (1 - endSize)} L ${x + w} ${y + h - (h / 2) * (1 - endSize)} L ${x} ${y + h - (h / 2) * (1 - startSize)} z`, style: `fill: ${channelColors.primaryNote};` }));
        } else {
          container.appendChild(SVG.rect({ x: x, y: y, width: w, height: h, style: `fill: ${channelColors.primaryNote};` }));
        }
        if (rawStart < 0) {
          const arrowY: number = y + h / 2, arrowHeight: number = Math.min(h, 20);
          const arrow: SVGPathElement = SVG.path({ d: `M ${prettyNumber(partWidth * stepStart + 2 + padding)} ${prettyNumber(arrowY - 0.1 * arrowHeight)} L ${prettyNumber(partWidth * stepStart + 2 + padding)} ${prettyNumber(arrowY + 0.1 * arrowHeight)} L ${prettyNumber(partWidth * stepStart + 6 + padding)} ${prettyNumber(arrowY + 0.1 * arrowHeight)} L ${prettyNumber(partWidth * stepStart + 6 + padding)} ${prettyNumber(arrowY + 0.3 * arrowHeight)} L ${prettyNumber(partWidth * stepStart + 14 + padding)} ${prettyNumber(arrowY)} L ${prettyNumber(partWidth * stepStart + 6 + padding)} ${prettyNumber(arrowY - 0.3 * arrowHeight)} L ${prettyNumber(partWidth * stepStart + 6 + padding)} ${prettyNumber(arrowY - 0.1 * arrowHeight)}`, fill: ColorConfig.invertedText });
