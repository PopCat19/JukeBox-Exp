...
export class EuclideanRhythmPrompt implements Prompt {
    ...
    for (let bar: number = firstBar; bar < lastBar; bar++) {
      ...
      for (let sequenceIndex: number = 0; sequenceIndex < this._sequences.length; sequenceIndex++) {
        ...
        if (generatedSequence.length !== steps) {
          console.error(
            "The size of the generated sequence and the specified number of steps it should take have diverged: generated",
        ...
        for (const eventGroup of eventGroups) {
          if (heldNotes.length === 0) {
            ...
            if (theNewNote != null) {
              ...
              const startRatio: number = (mergedStartPart - theNewNoteStartPart)
                / (theNewNoteEndPart - theNewNoteStartPart);
              const startPinSize: number = Math.round(startSize + (endSize - startSize) * startRatio);
              mergedNote.pins[0].size = startPinSize;
              const endRatio: number = (mergedEndPart - theNewNoteStartPart)
                / (theNewNoteEndPart - theNewNoteStartPart);
              ...
              if (firstVisibleOldPinIndex !== -1) {
                ...
                if (firstNewPin.time !== 0) {
                  ...
                  const newSize: number = Math.round(
                    leftAdjacentOldPin.size + (firstNewPin.size - leftAdjacentOldPin.size) * ratio,
                  );
                ...
                if (lastNewPin.time !== mergedNoteLength) {
                  ...
                  const lineMiddleTime: number = mergedEndRelativeToOldStart
                  ...
                  const ratio: number = lineMiddleTime / lineEndTime;
                  const newSize: number = Math.round(
                    lastNewPin.size + (rightAdjacentOldPin.size - lastNewPin.size) * ratio,
                  );
                ...
                const firstNewSize: number = Math.round(
                  leftAdjacentOldPin.size + (rightAdjacentOldPin.size - leftAdjacentOldPin.size) * firstRatio,
                );
                const lastNewSize: number = Math.round(
                  leftAdjacentOldPin.size + (rightAdjacentOldPin.size - leftAdjacentOldPin.size) * lastRatio,
                );
  ...
  private _renderSequenceButtons = (): void => {
    ...
    while (this._sequenceButtons.length < this._sequences.length) {
      const sequenceButton: HTMLButtonElement = button(
        { class: "no-underline" },
    ...
    const colors: ChannelColors = ColorConfig.getChannelColor(
      this._doc.song,
      this._sequences[this._sequenceIndex].channel,
    );
  ...
  private _renderLabel = (): void => {
    ...
    const pitchNameIndex: number = (sequencePitch + Config.keys[this._doc.song.key].basePitch)
    ...
    for (let step: number = firstStep; step < lastStep; step++) {
      ...
      if (hasGeneratedSequence && generatedSequence[step % steps] === on) {
        ...
        if (continuesLastPattern) {
          ...
          arrowPath = "M " + prettyNumber(partWidth * stepPartStart + indicatorOffset) + " "
            + prettyNumber(arrowY - 0.1 * arrowHeight);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset) + " "
            + prettyNumber(arrowY + 0.1 * arrowHeight);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset + 4) + " "
            + prettyNumber(arrowY + 0.1 * arrowHeight);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset + 4) + " "
            + prettyNumber(arrowY + 0.3 * arrowHeight);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset + 12) + " "
            + prettyNumber(arrowY);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset + 4) + " "
            + prettyNumber(arrowY - 0.3 * arrowHeight);
          arrowPath += "L " + prettyNumber(partWidth * stepPartStart + indicatorOffset + 4) + " "
