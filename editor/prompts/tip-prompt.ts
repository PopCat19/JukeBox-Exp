...
//
// This module:
// - Shows categorized tip entries for editor features
// - Handles tip navigation and display
    ...
    switch (type) {
      ...
      case "distortion":
        {
          message = div(
            ...
            p("Second, the distortion sound depends a lot on filtering. In particular, it is recommended to enable the note filter effect, and adding both high-pass and low-pass points to the note filter. (Note filters are applied first, then distortion which transforms the sound based on that filtering, then the EQ filter is applied last.)"),
            p("Finally, it is recommended to adjust the fade-out setting to allow the end of each note to overlap a little bit with the beginning of the next, but not too much!"),
          );
        }
        break;
      ...
      case "perEnvelopeSpeed":
        {
          message = div(
            ...
            p(
              ...
              HTML.a({ href: "./faq.html", target: "_blank" }, "FAQ"),
              " page",
            ),
      ...
      case "loopControls":
        {
          message = div(
            h2("Loop Controls"),
            p("This enables the use of parameters that control how a chip wave should repeat."),
          );
        }
        break;
      case "loopMode":
        {
          message = div(
            ...
            p("The \"Ping-Pong\" mode starts playing the chip wave backwards when the end of the loop is reached. Once it reaches the start of the loop, it will start playing forwards again, endlessly going back and forth."),
            p("The \"Play Once\" mode stops the chip wave once the end is reached (or the start of the loop, if it's playing backwards)."),
            p("The \"Play Loop Once\" mode stops the chip wave once the end of the loop is reached (or the start of the loop, if it's playing backwards)."),
          );
        }
        break;
      case "loopStart":
        {
          message = div(
            h2("Loop Start Point"),
            p("This specifies where the loop region of the chip wave starts. It's measured in \"samples\", or rather, it refers to a point on a waveform."),
            p("Be careful with tiny loop sizes (especially combined with high pitches), they may re-introduce aliasing even if the \"Aliasing\" checkbox is unchecked."),
          );
        }
        break;
      case "loopEnd":
        {
          message = div(
            h2("Loop End Point"),
            p("This specifies where the loop region of the chip wave ends. It's measured in \"samples\", or rather, it refers to a point on a waveform."),
      ...
      case "offset":
        {
          message = div(
            h2("Offset"),
            p("This specifies where the chip wave should start playing from. You can use this to chop up a large sample, to say, turn a drum loop into a drum kit! It's measured in \"samples\", or rather, it refers to a point on a waveform."),
          );
        }
        break;
      case "backwards":
        {
          message = div(
            h2("Backwards"),
            p("When set, the chip wave will start playing backwards. After checking this, you may want to adjust the offset to start from a different point that makes sense for this mode."),
          );
        }
        break;
      case "decimalOffset":
      ...
      default:
        ...
        if (type.indexOf("modSetInfo") >= 0) {
          const modNum: number = +type[type.length - 1];
          const modulator: number =
            _doc.song.channels[_doc.channel].instruments[_doc.getCurrentInstrument()].modulators[modNum];
