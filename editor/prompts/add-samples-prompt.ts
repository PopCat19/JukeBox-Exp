...
export class AddSamplesPrompt extends BasePrompt {
  ...
  private readonly _addSamplesAreaBottom: HTMLDivElement = div(
  ...
  private readonly _description: HTMLDivElement = div(
    div(
      {
        style:
          "margin-bottom: 0.5em; -webkit-user-select: text; -moz-user-select: text; -ms-user-select: text; user-select: text; cursor: text;",
      },
  ...
  private readonly _instructionsArea: HTMLDivElement = div(
    {
      style:
        "display: none; margin-top: 0; -webkit-user-select: text; -moz-user-select: text; -ms-user-select: text; user-select: text; cursor: text; overflow-y: auto;",
    },
    h2("Add Samples"),
    div(
  ...
  private readonly _bulkAddArea: HTMLDivElement = div(
    ...
    div(
      ...
      div(
        ...
        "(This supports the syntax used to store samples in the song URLs as well)",
      ),
  ...
  private _whenURLChanges = (event: Event): void => {
    ...
    const sampleNameElement: HTMLDivElement | null | undefined = element.parentNode?.parentNode?.querySelector(
      ".add-sample-prompt-sample-name",
    );
  ...
  private _whenRootKeyChanges = (event: Event): void => {
    ...
    const rootKeyDisplay: HTMLSpanElement | null | undefined = element.parentNode?.parentNode?.querySelector(
      ".add-sample-prompt-root-key-display",
    );
  ...
  private _generateURLDataForEntry = (entry: SampleEntry): string => {
    ...
    const isBundledSamplePack: boolean = urlInLowerCase === "legacysamples"
  ...
  private _render = (scrollToBottom: boolean): void => {
    ...
    for (let entryIndex: number = 0; entryIndex < this._entries.length; entryIndex++) {
      ...
      const rootKeyDisplay: HTMLSpanElement = span({
        ...
        style:
          "margin-left: 0.4em; width: 3em; text-align: left; text-overflow: ellipsis; overflow: hidden; flex-shrink: 0;",
      ...
      const optionsContainer: HTMLDetailsElement = details(
        ...
        div(
          {
            style:
              "display: flex; flex-direction: row; align-items: center; justify-content: space-between; margin-bottom: 0.5em;",
          },
          div(
            { style: `text-align: right; color: ${ColorConfig.primaryText};` },
            "Percussion (pitch doesn't change with key)",
          ),
          percussionBox,
        ),
        div(
      ...
      const entryElement: HTMLDivElement = div(
        {
          style:
            `padding: 0.6em; margin: 0.4em; border: 1px solid ${ColorConfig.uiWidgetBackground}; border-radius: 4px;`,
        },
        div({
          class: "add-sample-prompt-sample-name",
          style:
            `margin-bottom: 0.5em; color: ${ColorConfig.secondaryText}; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;`,
          title: sampleName,
        }, sampleName),
        div(
          {
