...
import { ChipWave, Config, sampleLoadingState, SampleLoadingStatus } from "../../synth/synth-config";
import {
...
class VisualLoopControlsHandle {
  ...
  constructor(
    ...
    this.canvas = canvas({
      ...
      style:
        "cursor: default; position: static; margin-bottom: 0.5em; margin-left: auto; margin-right: auto; outline: 1px solid var(--ui-widget-background); box-sizing: border-box; width: 100%;",
    });
...
export class VisualLoopControlsPrompt {
  ...
  private _waveformContainer: HTMLDivElement = div(
    {
      style:
        `position: relative; margin-bottom: 0.5em; margin-left: auto; margin-right: auto; width: 100%; outline: 1px solid ${ColorConfig.uiWidgetBackground};`,
    },
  ...
  private _loopControlsContainer: HTMLDivElement = div(
    div(
      {
        style:
          "display: flex; flex-direction: column; align-items: center; justify-content: center; margin-bottom: 0.5em;",
      },
      div(
    ...
    div(
      {
        style:
          "display: flex; flex-direction: row; align-items: center; justify-content: center; margin-bottom: 0.5em;",
      },
  ...
  constructor(_doc: SongDocument, _songEditor: SongEditor) {
    ...
    const customSampleIsLoading: boolean = (rawChipWave.isCustomSampled === true || rawChipWave.isSampled == true)
  ...
  private _whenZoomInClicked = (event: Event): void => {
    ...
    this._waveformViewportMaxOffset = this._waveformDataLength! - this._waveformViewportWidth;
    const centerX: number = this._waveformViewportX0
      + (this._waveformCanvasWidth / 2) * (this._waveformViewportX1 - this._waveformViewportX0)
        / this._waveformCanvasWidth;
    this._waveformViewportOffset = Math.max(
      0,
      Math.min(
        this._waveformViewportMaxOffset,
        centerX - (this._waveformCanvasWidth / 2) * this._waveformViewportWidth / this._waveformCanvasWidth,
      ),
    );
    this._waveformViewportX0 = 0 + this._waveformViewportOffset;
    this._waveformViewportX1 = this._waveformViewportWidth + this._waveformViewportOffset;
  ...
  private _whenZoomOutClicked = (event: Event): void => {
    ...
    this._waveformViewportOffset = Math.max(
      ...
      Math.min(
        this._waveformViewportMaxOffset,
        centerX - (this._waveformCanvas.width / 2) * this._waveformViewportWidth / this._waveformCanvasWidth,
      ),
    );
    this._waveformViewportX0 = 0 + this._waveformViewportOffset;
    this._waveformViewportX1 = this._waveformViewportWidth + this._waveformViewportOffset;
  ...
  private _whenOverlayMouseIsUp = (event: MouseEvent): void => {
    ...
    if (this._overlaySelectionX1 - this._overlaySelectionX0 > 2) {
      ...
      this._waveformViewportWidth = newViewportWidth;
      this._waveformViewportMaxOffset = this._waveformDataLength! - this._waveformViewportWidth;
      const centerX: number = vx0
        + (this._overlaySelectionX0) * (this._waveformViewportX1 - this._waveformViewportX0)
          / this._waveformCanvasWidth;
