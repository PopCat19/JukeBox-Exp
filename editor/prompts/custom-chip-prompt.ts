...
export class CustomChipPromptCanvas {
  ...
  constructor(doc: SongDocument) {
    ...
    for (let i: number = 0; i <= 64; i++) {
      const val: number =
        this._doc.song.channels[this._doc.channel].instruments[this._doc.getCurrentInstrument()].customChipWave[i];
      this.chipData[i] = val;
      ...
      if (this._lastIndex != -1 && this._lastIndex != index) {
        ...
        for (let i = lowest; i <= highest; i++) {
          const medAmp: number = Math.round(
            startingAmp + (endingAmp - startingAmp) * ((i - lowest) / (highest - lowest)),
          );
