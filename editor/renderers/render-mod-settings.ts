...
export function renderModSettings(
	...
	for (let mod: number = 0; mod < Config.modCount; mod++) {
		...
		// Boundary checking
		if (modInstrument >= doc.song.channels[modChannel].instruments.length + 2 || (modInstrument > 0 && doc.song.channels[modChannel].instruments.length <= 1)) {
			modInstrument = 0;
		...
		if (instrument.modChannels[mod] != -2) {
			...
			if (setIndex == -1) {
				refs.modSetBoxes[mod].insertBefore(option({ value: Config.modulators[instrument.modulators[mod]].name, style: "color: red;" }, Config.modulators[instrument.modulators[mod]].name), refs.modSetBoxes[mod].children[0]);
