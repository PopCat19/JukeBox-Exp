...
export function renderModSettings(
  ...
  for (let mod: number = 0; mod < Config.modCount; mod++) {
    ...
    if (
      ...
      || (refs.modChannelBoxes[mod].children.length != 2 + doc.song.pitchChannelCount + doc.song.noiseChannelCount)
    ) {
    ...
    if (instrument.modChannels[mod] < 0) {
      ...
      ((refs.modInstrumentBoxes[mod].parentElement) as HTMLDivElement).style.display = (channel.instruments.length > 1)
        ? ""
        : "none";
    ...
    if (
      envelopes == "individual envelope speed" || envelopes == "reset envelope"
      || envelopes == "individual envelope lower bound" || envelopes == "individual envelope upper bound"
    ) {
