...
export interface InstrumentValueRefs {
	...
	vibratoDepthInputBox: HTMLInputElement;
	vibratoDelayInputBox: HTMLInputElement;
	vibratoSpeedInputBox: HTMLInputElement;
	arpeggioSpeedInputBox: HTMLInputElement;
	panDelayInputBox: HTMLInputElement;
	eqFilterSimpleCutInputBox: HTMLInputElement;
	eqFilterSimplePeakInputBox: HTMLInputElement;
	noteFilterSimpleCutInputBox: HTMLInputElement;
	noteFilterSimplePeakInputBox: HTMLInputElement;
	envelopeSpeedInputBox: HTMLInputElement;
...
export function renderInstrumentValues(
	...
	refs.vibratoDepthInputBox.value = `${Math.round(instrument.vibratoDepth * 25)}`;
	refs.vibratoDelaySlider.updateValue(Math.round(instrument.vibratoDelay));
	refs.vibratoDelayInputBox.value = `${Math.round(instrument.vibratoDelay)}`;
	refs.vibratoSpeedSlider.updateValue(instrument.vibratoSpeed);
	refs.vibratoSpeedInputBox.value = `${instrument.vibratoSpeed}`;
	setSelectedValue(refs.vibratoTypeSelect, instrument.vibratoType);
	refs.arpeggioSpeedSlider.updateValue(instrument.arpeggioSpeed);
	refs.arpeggioSpeedInputBox.value = `${instrument.arpeggioSpeed}`;
	refs.panDelaySlider.updateValue(instrument.panDelay);
	refs.panDelayInputBox.value = `${instrument.panDelay}`;
	...
	refs.eqFilterSimpleCutInputBox.value = `${instrument.eqFilterSimpleCut}`;
	refs.eqFilterSimplePeakSlider.updateValue(instrument.eqFilterSimplePeak);
	refs.eqFilterSimplePeakInputBox.value = `${instrument.eqFilterSimplePeak}`;
	...
	refs.envelopeSpeedInputBox.value = `${instrument.envelopeSpeed}`;
