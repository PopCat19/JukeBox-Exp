...
export function applyInstrumentVisibility(
  ...
  if (effectsIncludeEcho(instrument.effects)) {
    ...
    refs.echoDelaySlider.input.title = (Math.round(
      (instrument.echoDelay + 1) * Config.echoDelayStepTicks / (Config.ticksPerPart * Config.partsPerBeat) * 1000,
    ) / 1000) + " beat(s)";
  } else {
  ...
  for (let chordIndex: number = 0; chordIndex < Config.chords.length; chordIndex++) {
    const hidden: boolean = !Config.instrumentTypeHasSpecialInterval[instrument.type]
