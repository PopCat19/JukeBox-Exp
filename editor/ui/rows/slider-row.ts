...
export function sliderRow(options: SliderRowOptions, slider: Slider): HTMLDivElement {
	...
	return div({ class: "selectRow", style: options.style }, span({ ...labelAttrs, onclick: options.onClick }, options.label), slider.container);
}

export function sliderRowWithInput(options: SliderRowWithInputOptions, slider: Slider): HTMLDivElement {
	const labelAttrs: Record<string, string> = { class: "tip" };
	if (options.labelStyle) {
		labelAttrs.style = options.labelStyle;
	}

	const labelSpan = options.onClick ? span({ ...labelAttrs, onclick: options.onClick }, options.label) : span({ ...labelAttrs }, options.label);
