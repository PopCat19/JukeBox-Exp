// Prompts - Index
//
// Purpose: Barrel re-export of prompt layout components

export { type InfoBannerOptions, infoBanner } from "./info-banner";
export { type InputRowOptions, inputRow } from "./input-row";
export { type InstructionsOptions, instructions } from "./instructions";
export { fixedPane, flexPane, type PaneOptions, pane } from "./pane";
export { type PaneContainerOptions, paneContainer } from "./pane-container";
export {
	promptHint,
	promptLabel,
	promptRowBetween,
	promptRowEnd,
	promptValue,
} from "./prompt-components";
