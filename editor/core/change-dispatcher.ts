...
import { pickNextPresetValue, pickRandomPresetValue } from "../changes";
import {
  Change6OpAlgorithm,
...
export class ChangeDispatcher {
  ...
  public whenSetModChannel = (mod: number): void => {
    ...
    const previouslyUnset: boolean = instrument.modulators[mod] == 0
    ...
    if (
      this.doc.song.channels[modChannel].instruments.length > 1 && previouslyUnset
      ...
      if (this.doc.song.channels[modChannel].bars[this.doc.bar] > 0) {
        this.doc.selection.setModInstrument(
          mod,
          this.doc.song.channels[modChannel].patterns[this.doc.song.channels[modChannel].bars[this.doc.bar] - 1]
            .instruments[0],
        );
      }
    }
  ...
  public whenSetModSetting = (mod: number, invalidIndex: boolean = false): void => {
    ...
    if (this._host.modSetBoxes[mod].selectedIndex != -1) {
      ...
      if (invalidIndex) {
        ...
        this.doc.song.channels[this.doc.channel].instruments[this.doc.getCurrentInstrument()].invalidModulators[mod] =
          true;
      } else {
  ...
  public whenSetUseChipWaveAdvancedLoopControls = (): void => {
    this.doc.record(
      new ChangeChipWaveUseAdvancedLoopControls(
        this.doc,
        this._host.useChipWaveAdvancedLoopControlsBox.checked ? true : false,
      ),
    );
  };
  ...
  public pasteInstrument = (): void => {
    ...
    if (
      instrumentCopy != null && instrumentCopy["isDrum"] == this.doc.song.getChannelIsNoise(this.doc.channel)
      && instrumentCopy["isMod"] == this.doc.song.getChannelIsMod(this.doc.channel)
    ) {
