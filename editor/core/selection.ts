...
export class Selection {
	...
	public nextDigit(digit: string, forInstrument: boolean, forRhythms: boolean): void {
		if (forRhythms) {
			...
			this.digits += digit;
			let parsed: number = parseInt(this.digits, 10);
			...
			this.digits = digit;
			parsed = parseInt(this.digits, 10);
			if (parsed <= this._doc.song.patternsPerChannel) {
				this.setPattern(parsed);
	...
	private _reconcilePastedInstruments(channelCopy: ChannelCopy, channelIndex: number): number[] {
		...
		for (const [srcIdxStr, instDef] of Object.entries(channelCopy.instrumentDefs)) {
			...
			// No match — append the instrument. Ignore maxInstruments:
			// it's a soft UI limit, not a hard cap. Cross-project paste
			// must be able to bring instruments over.
