...
export class Selection {
  ...
  public scrollToEndOfSelection(): void {
    this._doc.barScrollPos = Math.min(
      this.boxSelectionX1,
      Math.max(this.boxSelectionX1 - (this._doc.trackVisibleBars - 1), this._doc.barScrollPos),
    );
    this._doc.channelScrollPos = Math.min(
      this.boxSelectionY1,
  ...
  public insertBars(): void {
    this._doc.record(
      new ChangeInsertBars(this._doc, this.boxSelectionBar + this.boxSelectionWidth, this.boxSelectionWidth),
    );
  ...
  private _convertCopiedPitchNotesToNoiseNotes(oldNotes: Note[]): Note[] {
    ...
    // there may be pitches beyond what a noise channel supports.
    const newNotes: Note[] = [];
  ...
  public pasteNotes(): void {
    ...
    for (let pasteChannel: number = 0; pasteChannel < pasteHeight; pasteChannel++) {
      ...
      if (!fillSelection && copiedBars.length == 1 && channelCopies.length == 1) {
        ...
        if (
          currentPatternIndex == 0 || patternCopy.notes.length == 0
        ...
        for (let pasteBar: number = 0; pasteBar < pasteWidth; pasteBar++) {
          ...
          if (copiedPatternIndex == 0) {
            group.append(
              new ChangeNoteTruncate(this._doc, pattern, this.patternSelectionStart, this.patternSelectionEnd),
            );
          } else {
            const patternCopy: PatternCopy = patternCopies[String(copiedPatternIndex)];
            let pastedNotes: Note[] = patternCopy["notes"];
  ...
  public soloChannels(invert: boolean): void {
    ...
    if (this.boxSelectionChannel >= this._doc.song.pitchChannelCount + this._doc.song.noiseChannelCount) {
      ...
      for (
        ...
        const shouldBeMuted: boolean = (channelIndex < this.boxSelectionChannel
            || channelIndex >= this.boxSelectionChannel + this.boxSelectionHeight)
          ? !invert
          : invert;
      ...
      if (alreadySoloed) {
        ...
        for (
          ...
          this._doc.song.channels[channelIndex].muted = (channelIndex < this.boxSelectionChannel
              || channelIndex >= this.boxSelectionChannel + this.boxSelectionHeight)
            ? !invert
            : invert;
        }
  ...
  public transpose(upward: boolean, octave: boolean): void {
    ...
    for (const channelIndex of this._eachSelectedChannel()) {
      ...
      for (const pattern of this._eachSelectedPattern(channelIndex)) {
        this._changeTranspose.append(
          new ChangeTranspose(this._doc, channelIndex, pattern, upward, this._doc.prefs.notesOutsideScale, octave),
        );
      }
    }
    ...
    if (offset != 0) {
      ...
      this._changeReorder.append(
        new ChangeChannelBar(
          this._doc,
          Math.max(this.boxSelectionY0, Math.min(this.boxSelectionY1, this._doc.channel + offset)),
          this._doc.bar,
        ),
      );
      this.selectionUpdated();
  ...
  public selectInstrument(instrument: number): void {
    if (this._doc.viewedInstrument[this._doc.channel] == instrument) {
      ...
      if (
        this._doc.song.layeredInstruments && this._doc.song.patternInstruments
        ...
        for (const channelIndex of this._eachSelectedChannel()) {
          for (const pattern of this._eachSelectedPattern(channelIndex)) {
            this._changeInstrument.append(
              new ChangeSetPatternInstruments(this._doc, channelIndex, instruments, pattern),
            );
          }
        }
