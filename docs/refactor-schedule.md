...
| 4.2 Extract modulator resolution | New `synth/mod-state.ts` (490 lines) | ✓ Extracted SynthModState class owning modValues/modInsValues/heldMods, plus all accessor+mutator methods (setModValue, getModValue, isAnyModActive, isModActive, unset, forceHoldMods, findPartsInBar, computeLatestModValues, determineInvalidModulators, initModFilters, advanceNextToValues, filterModForSong/Eq/Note). synth.ts reduced from 4871→4423 lines (−448). All 173 tests pass, typecheck clean. |
