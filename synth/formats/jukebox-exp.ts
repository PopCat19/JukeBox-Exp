...
export function fromJukeboxExpJson(
  song: SongLike,
  obj: JukeboxExpObject,
): void {
  const expVersion = obj._expVersion ?? 0;
  if (
    expVersion < JUKEBOX_EXP_OLDEST_VERSION ||
    expVersion > JUKEBOX_EXP_LATEST_VERSION
  ) {
    return;
  }
