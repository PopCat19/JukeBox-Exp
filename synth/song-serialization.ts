...
export function toBase64StringImpl(song: SongLike): string {
  ...
  if (
    song.compressionRatio != 1.0 || song.limitRatio != 1.0 || song.limitRise != 4000.0 || song.limitDecay != 4.0
    || song.limitThreshold != 1.0 || song.compressionThreshold != 1.0 || song.masterGain != 1.0
  ) {
    buffer.push(
      base64IntToCharCode[
        Math.round(song.compressionRatio < 1 ? song.compressionRatio * 10 : 10 + (song.compressionRatio - 1) * 60)
      ],
    ...
    for (let i: number = 0; i < song.channels[channelIndex].instruments.length; i++) {
      ...
      if (instrument.eqFilterType) {
        ...
        if (instrument.eqFilter == null) {
          ...
          buffer.push(base64IntToCharCode[0]);
          console.log(
            "Null EQ filter settings detected in toBase64String for channelIndex " + channelIndex + ", instrumentIndex "
              + i,
          );
        } else {
      ...
      if (effectsIncludeNoteFilter(instrument.effects)) {
        ...
        if (instrument.noteFilterType) {
          ...
          if (instrument.noteFilter == null) {
            ...
            buffer.push(base64IntToCharCode[0]);
            console.log(
              "Null note filter settings detected in toBase64String for channelIndex " + channelIndex
                + ", instrumentIndex " + i,
            );
          } else {
      ...
      if (instrument.type == InstrumentType.chip) {
        ...
        const encodedLoopMode: number = (clamp(0, 31 + 1, instrument.chipWaveLoopMode) << 1)
        ...
        const encodedReleaseMode: number = (clamp(0, 31 + 1, 0) << 1)
        ...
        buffer.push(
          ...
          base64IntToCharCode[instrument.stringSustain | (instrument.stringSustainType << 5)],
        );
      ...
      for (let envelopeIndex: number = 0; envelopeIndex < instrument.envelopeCount; envelopeIndex++) {
        ...
        if (
          ...
          buffer.push(
            base64IntToCharCode[Config.perEnvelopeSpeedToIndices[instrument.envelopes[envelopeIndex].perEnvelopeSpeed]],
          );
        }
  ...
  for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) {
    ...
    const recentPitches: number[] = isModChannel
      ? [0, 1, 2, 3, 4, 5]
      : (isNoiseChannel ? [4, 6, 7, 2, 3, 8, 0, 10] : [0, 7, 12, 19, 24, -5, -12]);
...
export function fromBase64StringImpl(song: SongLike, compressed: string, jsonFormat: string = "auto"): void {
  ...
  if (
    fromSlarmoosBox && (version == -1 || version > LATEST_SLARMOOSBOX_VERSION || version < OLDEST_SLARMOOSBOX_VERSION)
  ) return;
  ...
  while (charIndex < compressed.length) {
    switch (compressed.charCodeAt(charIndex++)) {
      case SongTagCode.songTitle:
        {
          ...
          const songNameLength = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
      ...
      case SongTagCode.key:
        {
          if (beforeSeven && fromBeepBox) {
            ...
            song.octave = clamp(
              Config.octaveMin,
              Config.octaveMax + 1,
              base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + Config.octaveMin,
            );
          }
        }
        break;
      case SongTagCode.loopStart:
        {
          if (beforeFive && fromBeepBox) {
            ...
            song.loopStart = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
              + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
          }
        }
        break;
      case SongTagCode.loopEnd:
        {
          if (beforeFive && fromBeepBox) {
            ...
            song.loopLength = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
      ...
      case SongTagCode.tempo:
        {
          if (beforeFour && fromBeepBox) {
            ...
            song.tempo = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
              | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
          }
      ...
      case SongTagCode.barCount:
        {
          const barCount: number = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
            + base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + 1;
      ...
      case SongTagCode.patternCount:
        {
          ...
          if (beforeEight && fromBeepBox) {
            ...
            patternsPerChannel = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
              + base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + 1;
          }
      ...
      case SongTagCode.instrumentCount:
        {
          if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
            ...
            for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) {
              const isNoiseChannel: boolean = channelIndex >= song.pitchChannelCount
      ...
      case SongTagCode.startInstrument:
        {
          ...
          if ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox)) {
            ...
            if (
              instrumentType == InstrumentType.supersaw || instrumentType == InstrumentType.customChipWave
          ...
          if (
            ...
            && (instrumentType == InstrumentType.chip || instrumentType == InstrumentType.customChipWave
      ...
      case SongTagCode.preset:
        {
          ...
          if (!fromJukeBox) {
            presetValue = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 12)
              | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
          } else {
            presetValue = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 18)
          ...
          if ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox)) {
            if (
              ...
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].type =
                InstrumentType.customChipWave;
            }
            ...
            if (
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset == InstrumentType.mod
            ) {
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset =
          ...
          if (fromBeepBox && presetValue == song.customSampleHandler?.nameToPresetValue("grand piano 1")) {
            song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset = song
              .customSampleHandler?.nameToPresetValue("grand piano 3")!;
          }
        }
        break;
      case SongTagCode.wave:
        {
          if (beforeThree && fromBeepBox) {
            ...
            if (
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].type == InstrumentType.noise
            ) {
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipNoise = clamp(
                0,
                Config.chipNoises.length,
      ...
      case SongTagCode.eqFilter:
        {
          if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
            if (beforeSeven && fromBeepBox) {
              ...
              if (beforeThree && fromBeepBox) {
                ...
                const instrument: Instrument =
                  song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
            ...
            if (fromBeepBox || typeCheck == 0) {
              ...
              if (
                (fromJummBox && !beforeFive) || (fromGoldBox && !beforeFour) || fromUltraBox || fromSlarmoosBox
                || fromJukeBox
              ) {
                const usingSubFilterBitfield: number = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                  | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                for (let j: number = 0; j < Config.filterMorphCount - 1; j++) {
                  if (usingSubFilterBitfield & (1 << j)) {
          ...
          if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
            ...
            if (instrument.type == InstrumentType.drumset) {
              for (let i: number = 0; i < Config.drumCount; i++) {
                ...
                if (
                  (beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromJukeBox)
      ...
      case SongTagCode.pulseWidth:
        {
          ...
          if (fromBeepBox) {
            ...
            instrument.pulseWidth = Math.round(
              Math.pow(0.5, (7 - instrument.pulseWidth) * Config.pulseWidthStepPower) * Config.pulseWidthRange,
            );
          }
          ...
          if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
            ...
            if (beforeThree && fromBeepBox) {
              ...
              const settings = legacySettings[
                clamp(0, legacySettings.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])
              ];
              const instrument: Instrument = song.channels[channelIndex].instruments[0];
              ...
              (beforeFour && !fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromJukeBox) || fromBeepBox
            ) {
              const settings = legacySettings[
                clamp(0, legacySettings.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])
              ];
              ...
              if (
                instrument.transition != Config.transitions.dictionary["normal"].index || instrument.clicklessTransition
              ) {
                // Enable transition if it was used.
                instrument.effects |= 1 << EffectType.transition;
              }
          ...
          if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
            if (beforeSeven && fromBeepBox) {
              if (beforeThree && fromBeepBox) {
                ...
                if (
                  legacySettings.filterEnvelope == undefined || legacySettings.filterEnvelope.type == EnvelopeType.none
                ) {
            ...
            if (fromJukeBox || (fromSlarmoosBox && !beforeFour)) { // double check that it's from a valid version
              ...
              for (let j: number = 0; j < Config.filterMorphCount - 1; j++) {
                if (usingSubFilterBitfield & (1 << j)) {
                  ...
                  for (
                    let i: number = song.eqSubFilters[j + 1]!.controlPoints.length;
      ...
      case SongTagCode.unison:
        {
          if (beforeThree && fromBeepBox) {
            ...
            if (
              ((fromUltraBox && !beforeFive) || fromSlarmoosBox || fromJukeBox) && (instrument.unison == unisonLength)
            ) {
              // if (instrument.unison == Config.unisons.length) {
              ...
              const unisonSpread: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
                + ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
                  + (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] * 63)) * 63);

              const unisonOffsetNegative = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              const unisonOffset: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
                + ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
                  + (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] * 63)) * 63);

              const unisonExpressionNegative = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              const unisonExpression: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
              ...
              const unisonSign: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
                + (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] * 63);
      ...
      case SongTagCode.effects:
        {
          const instrument: Instrument = song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
          if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
            instrument.effects = base64CharCodeToInt[compressed.charCodeAt(charIndex++)]
            ...
            if (fromJukeBox || (fromSlarmoosBox && !beforeFive)) {
              instrument.effects = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 12)
                | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
            } else {
              instrument.effects = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
            }
            ...
            if (effectsIncludeDetune(instrument.effects)) {
              if (fromBeepBox) {
                ...
                instrument.detune = Math.round(
                  (instrument.detune - 9) * (Math.abs(instrument.detune - 9) + 1) / 2 + Config.detuneCenter,
                );
              } else {
                instrument.detune = clamp(
                  Config.detuneMin,
            ...
            if (effectsIncludeChorus(instrument.effects)) {
              if (fromBeepBox) {
                ...
                instrument.chorus =
                  clamp(0, (Config.chorusRange / 2) + 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]) * 2;
              } else {
            ...
            if (effectsIncludeNoteRange(instrument.effects)) {
              instrument.upperNoteLimit = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              instrument.lowerNoteLimit = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
            }
          }
          // Clamp the range.
          ...
          if (beforeThree && fromBeepBox) {
            ...
            instrument.volume = Math.round(
              clamp(
                ...
                ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                  | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)])) - Config.volumeRange / 2,
              ),
            );
          }
        }
      ...
      case SongTagCode.pan:
        {
          if (beforeNine && fromBeepBox) {
            ...
            instrument.pan = clamp(
              0,
              Config.panMax + 1,
              base64CharCodeToInt[compressed.charCodeAt(charIndex++)] * ((Config.panMax) / 8.0),
            );
          } else if ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox)) {
            const instrument: Instrument =
              song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
      ...
      case SongTagCode.detune:
        {
          ...
          if ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox)) {
            ...
            instrument.detune = clamp(
              ...
              Config.detuneMax + 1,
              ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)]) * 4,
            );
            instrument.effects |= 1 << EffectType.detune;
          } else {
      ...
      case SongTagCode.limiterSettings:
        {
          ...
          if (nextValue == 0x3f) {
            ...
            song.masterGain = ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
              + base64CharCodeToInt[compressed.charCodeAt(charIndex++)]) / 50.0;
          }
        }
        break;
      case SongTagCode.channelNames:
        {
          for (let channel: number = 0; channel < song.getChannelCount(); channel++) {
            ...
            if (beforeFour && !fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromJukeBox) {
              ...
              channelNameLength = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6)
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
            }
      ...
      case SongTagCode.operatorFrequencies:
        {
          ...
          if (beforeThree && fromGoldBox) {
            ...
            for (let o = 0; o < (instrument.type == InstrumentType.fm6op ? 6 : Config.operatorCount); o++) {
              instrument.operators[o].frequency = freqToUltraBox[
                clamp(0, freqToUltraBox.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])
              ];
            }
          } else {
            for (let o = 0; o < (instrument.type == InstrumentType.fm6op ? 6 : Config.operatorCount); o++) {
      ...
      case SongTagCode.envelopes:
        {
          ...
          if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
            ...
            for (let i: number = 0; i < envelopeCount; i++) {
              ...
              perEnvelopeSpeed = (fromJukeBox || (fromSlarmoosBox && !beforeThree))
              ...
              if (fromJukeBox || (fromSlarmoosBox && !beforeFour)) {
                if (Config.newEnvelopes[envelope].name == "lfo") {
                  ...
                  waveform = clamp(
                    ...
                    base64CharCodeToInt[compressed.charCodeAt(charIndex++)],
                  ); // we use waveform for the random type as well
                }
              }
              if (fromJukeBox || (fromSlarmoosBox && !beforeThree)) {
                ...
                if (
                  Config.newEnvelopes[envelope].name != "pitch" && Config.newEnvelopes[envelope].name != "note size"
                  && Config.newEnvelopes[envelope].name != "punch" && Config.newEnvelopes[envelope].name != "none"
                ) {
                  perEnvelopeSpeed =
                    Config.perEnvelopeSpeedIndices[base64CharCodeToInt[compressed.charCodeAt(charIndex++)]];
                }
              ...
              if ((!fromSlarmoosBox && !fromJukeBox) || beforeFour) { // update tremolo2
                if (isTremolo2) {
                  ...
                  if (envelopeInverse) {
                    ...
                    perEnvelopeUpperBound = Math.floor((0.5 + (perEnvelopeUpperBound - perEnvelopeLowerBound) / 2) * 10)
                      / 10;
              ...
              if (fromSlarmoosBox && beforeThree && !beforeTwo) {
                let pitchEnvelopeCompact: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                instrument.envelopes[i].pitchEnvelopeStart = pitchEnvelopeCompact * 64
                  + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                pitchEnvelopeCompact = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                instrument.envelopes[i].pitchEnvelopeEnd = pitchEnvelopeCompact * 64
                  + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
            ...
            if (fromSlarmoosBox && beforeTwo) {
              let pitchEnvelopeCompact: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              instrumentPitchEnvelopeStart = pitchEnvelopeCompact * 64
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              pitchEnvelopeCompact = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              instrumentPitchEnvelopeEnd = pitchEnvelopeCompact * 64
                + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
              ...
              for (let i: number = 0; i < envelopeCount; i++) {
                ...
                instrument.envelopes[i].inverse = Config.envelopes[instrument.envelopes[i].envelope].name == "pitch"
          ...
          if (beforeThree && fromGoldBox) {
            ...
            for (let o: number = 0; o < (instrument.type == InstrumentType.fm6op ? 6 : Config.operatorCount); o++) {
              if (fromJummBox) {
                ...
                instrument.operators[o].waveform = jummToG[
                  clamp(0, Config.operatorWaves.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])
                ];
              } else {
          ...
          while (true) {
                ...
                for (let mod: number = 0; mod < Config.modCount; mod++) {
                  ...
                  switch (status) {
                    ...
                    case 1: // Noise
                      ...
                      instrument.modChannels[mod] = song.pitchChannelCount
                  ...
                  if (jumfive && Config.modulators[instrument.modulators[mod]].associatedEffect != EffectType.length) {
                    song.channels[instrument.modChannels[mod]].instruments[instrument.modInstruments[mod]].effects |= 1
            ...
            for (let j: number = 0; j < channel.instruments.length; j++) {
              ...
              for (let i: number = 0; i < Config.modCount; i++) {
                detuneScaleNotes[j][Config.modCount - 1 - i] = 1
                  + 3
                    * +(((beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) && isModChannel
            ...
            for (let i: number = 0; i < song.patternsPerChannel; i++) {
              ...
              while (curPart < song.beatsPerBar * Config.partsPerBeat + (+isModChannel)) {
                ...
                if (!useOldShape && !newNote) {
                  ...
                  const noteIsForTempoMod: boolean = isModChannel
              ...
              for (
                ...
                if (songReverbChannel == channelIndex && songReverbInstrument == instrumentIndex) {
                  ...
                  if (patternIndex > 0) {
                    ...
                    if (lowestPart > 0) {
                      pattern.notes.push(
                        new Note(Config.modCount - 1 - songReverbIndex, 0, lowestPart, legacyGlobalReverb),
                      );
                    }
                  } else {
                    // Add pattern
...
export function fromJsonObjectImpl(song: SongLike, jsonObject: any, jsonFormat: string = "auto"): void {
  ...
  if (jsonObject["scale"] != undefined) {
    ...
    const scaleName: string = (oldScaleNames[jsonObject["scale"]] != undefined)
  ...
  if (jsonObject["channels"] != undefined) {
    for (let channelIndex: number = 0; channelIndex < jsonObject["channels"].length; channelIndex++) {
      ...
      for (let i: number = 0; i < song.barCount; i++) {
        channel.bars[i] = (channelObject["sequence"] != undefined)
