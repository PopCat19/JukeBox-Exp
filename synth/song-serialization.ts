...
export function toBase64StringImpl(song: SongLike): string {
    ...
    buffer.push(SongTagCode.channelCount, base64IntToCharCode[song.pitchChannelCount], base64IntToCharCode[song.noiseChannelCount], base64IntToCharCode[song.modChannelCount]);
    ...
    if (song.compressionRatio != 1.0 || song.limitRatio != 1.0 || song.limitRise != 4000.0 || song.limitDecay != 4.0 || song.limitThreshold != 1.0 || song.compressionThreshold != 1.0 || song.masterGain != 1.0) {
    ...
    for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) for (let i: number = 0; i < song.barCount; i++) {
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
            ...
            case SongTagCode.loopStart: {
                if (beforeFive && fromBeepBox) {
                    ...
                    song.loopStart = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) + base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                }
            } break;
            case SongTagCode.loopEnd: {
                if (beforeFive && fromBeepBox) {
                    ...
                    song.loopLength = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) + base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + 1;
                }
            } break;
            case SongTagCode.tempo: {
                if (beforeFour && fromBeepBox) {
                    ...
                    song.tempo = [88, 95, 103, 111, 120, 130, 140, 151, 163, 176, 190, 206, 222, 240, 259][base64CharCodeToInt[compressed.charCodeAt(charIndex++)]];
                } else {
            ...
            case SongTagCode.instrumentCount: {
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) {
                        const isNoiseChannel: boolean = channelIndex >= song.pitchChannelCount && channelIndex < song.pitchChannelCount + song.noiseChannelCount;
                        const isModChannel: boolean = channelIndex >= song.pitchChannelCount + song.noiseChannelCount;

                        for (let instrumentIndex: number = song.channels[channelIndex].instruments.length; instrumentIndex < instrumentsPerChannel; instrumentIndex++) {
                        ...
                        if (beforeSix && fromBeepBox) {
                            for (let instrumentIndex: number = 0; instrumentIndex < instrumentsPerChannel; instrumentIndex++) {
                                song.channels[channelIndex].instruments[instrumentIndex].setTypeAndReset(isNoiseChannel ? InstrumentType.noise : InstrumentType.chip, isNoiseChannel, isModChannel);
                            }
                        }
                    ...
                    for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) {
                        ...
                        if (song.layeredInstruments || song.patternInstruments) {
                            instrumentCount = validateRange(Config.instrumentCountMin, song.getMaxInstrumentsPerChannel(), base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + Config.instrumentCountMin);
                        }
                        const channel: Channel = song.channels[channelIndex];
                        const isNoiseChannel: boolean = song.getChannelIsNoise(channelIndex);
                        const isModChannel: boolean = song.getChannelIsMod(channelIndex);
            ...
            case SongTagCode.startInstrument: {
                ...
                instrument.setTypeAndReset(instrumentType, instrumentChannelIterator >= song.pitchChannelCount && instrumentChannelIterator < song.pitchChannelCount + song.noiseChannelCount, instrumentChannelIterator >= song.pitchChannelCount + song.noiseChannelCount);
            ...
            case SongTagCode.preset: {
                ...
                if (fromBeepBox && presetValue == song.customSampleHandler?.nameToPresetValue("grand piano 1")) {
                    song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].preset = song.customSampleHandler?.nameToPresetValue("grand piano 3")!;
                }
            } break;
            case SongTagCode.wave: {
                if (beforeThree && fromBeepBox) {
                    ...
                    if (instrumentChannelIterator >= song.pitchChannelCount) {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipNoise = clamp(0, Config.chipNoises.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                    } else {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, legacyWaves[base64CharCodeToInt[compressed.charCodeAt(charIndex++)]] | 0);
                    }
                } else {
                    if (song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].type == InstrumentType.noise) {
                        ...
                        if (fromJukeBox || fromSlarmoosBox || fromUltraBox) {
                            ...
                            if (chipWaveCounter == 3) {
                                song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveReal + 186);
                            } else if (chipWaveCounter == 2) {
                                song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveReal + 124);
                            } else if (chipWaveCounter == 1) {
                                song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveReal + 62);
                            } else {
                                song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveReal);
                            }

                        } else {
                            song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                        }
                    }
                }
            } break;
            ...
            case SongTagCode.loopControls: {
                if (fromJukeBox || fromSlarmoosBox || fromUltraBox) {
                    ...
                    song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + 125);
                } else if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
            ...
            case SongTagCode.limiterSettings: {
                ...
                if (nextValue == 0x3f) {
                    ...
                    song.masterGain = ((base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) + base64CharCodeToInt[compressed.charCodeAt(charIndex++)]) / 50.0;
                }
            } break;
            ...
            case SongTagCode.supersaw: {
                if (fromGoldBox && !beforeFour && beforeSix) {
                    ...
                    if ((chipWaveForCompat + 62) > 78) {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveForCompat + 63);
                    }
                    else if ((chipWaveForCompat + 62) > 67) {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveForCompat + 61);
                    }
                    else if ((chipWaveForCompat + 62) == 67) {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = 40;
                    }
                    else {
                        song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].chipWave = clamp(0, Config.chipWaves.length, chipWaveForCompat + 62);
                    }
                } else {
                    const instrument: Instrument = song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
            ...
            case SongTagCode.feedbackAmplitude: {
                song.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].feedbackAmplitude = clamp(0, Config.operatorAmplitudeMax + 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
            } break;
            ...
            case SongTagCode.patterns: {
                ...
                while (true) {
                    ...
                    if (isModChannel) {
                        ...
                        const neededModInstrumentIndexBits: number = (jumfive) ? neededInstrumentIndexBits : getNeededBits(song.getMaxInstrumentsPerChannel() + 2);
                            ...
                            for (let mod: number = 0; mod < Config.modCount; mod++) {
                                ...
                                switch (status) {
                                    case 0: // Pitch
                                        ...
                                        instrument.modInstruments[mod] = clamp(0, song.channels[instrument.modChannels[mod]].instruments.length + 2, bits.read(neededModInstrumentIndexBits));
                                        break;
                                    case 1: // Noise
                                        ...
                                        instrument.modInstruments[mod] = clamp(0, song.channels[instrument.modChannels[mod]].instruments.length + 2, bits.read(neededInstrumentIndexBits));
                                        break;
                                    case 2: // For song
                                ...
                                if (jumfive && instrument.modChannels[mod] >= 0) {
                                    const forNoteFilter: boolean = effectsIncludeNoteFilter(song.channels[instrument.modChannels[mod]].instruments[instrument.modInstruments[mod]].effects);
                                ...
                                if (jumfive && Config.modulators[instrument.modulators[mod]].associatedEffect != EffectType.length) {
                                    song.channels[instrument.modChannels[mod]].instruments[instrument.modInstruments[mod]].effects |= 1 << Config.modulators[instrument.modulators[mod]].associatedEffect;
                                }
                            }
                        }
                    }
...
export function toJsonObjectImpl(song: SongLike, enableIntro: boolean = true, loopCount: number = 1, enableOutro: boolean = true): object {
        ...
        for (let channelIndex: number = 0; channelIndex < song.getChannelCount(); channelIndex++) {
            ...
            if (enableIntro) for (let i: number = 0; i < song.loopStart; i++) {
                sequenceArray.push(channel.bars[i]);
            }
            for (let l: number = 0; l < loopCount; l++) for (let i: number = song.loopStart; i < song.loopStart + song.loopLength; i++) {
                sequenceArray.push(channel.bars[i]);
            }
            if (enableOutro) for (let i: number = song.loopStart + song.loopLength; i < song.barCount; i++) {
                sequenceArray.push(channel.bars[i]);
            }
