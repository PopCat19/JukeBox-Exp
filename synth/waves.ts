  ...
  public reset(isNoiseChannel: boolean): void {
    for (let i: number = 0; i < Config.spectrumControlPoints; i++) {
      if (isNoiseChannel) {
        ...
        const isHarmonic: boolean = i == 0 || i == 7 || i == 11 || i == 14 || i == 16 || i == 18 || i == 21 || i == 23
          || i >= 25;
...
export class SpectrumWaveState {
  ...
  public getCustomWave(settings: SpectrumWave, lowestOctave: number): Float32Array {
    ...
    function controlPointToOctave(point: number): number {
      return lowestOctave + Math.floor(point / Config.spectrumControlPointsPerOctave)
    ...
    for (let i: number = 0; i < Config.spectrumControlPoints + 1; i++) {
      ...
      const value2: number = (i >= Config.spectrumControlPoints)
...
export class HarmonicsWaveState {
  ...
  public getCustomWave(settings: HarmonicsWave, instrumentType: InstrumentType): Float32Array {
    ...
    const harmonicsRendered: number = (instrumentType == InstrumentType.pickedString)
    ...
    for (let harmonicIndex: number = 0; harmonicIndex < harmonicsRendered; harmonicIndex++) {
      ...
      let controlValue: number = harmonicIndex < Config.harmonicsControlPoints
        ? settings.harmonics[harmonicIndex]
        : settings.harmonics[Config.harmonicsControlPoints - 1];
      if (harmonicIndex >= Config.harmonicsControlPoints) {
        controlValue *= 1
          - (harmonicIndex - Config.harmonicsControlPoints) / (harmonicsRendered - Config.harmonicsControlPoints);
      }
...
export class Grain {
  ...
  public updateRCBEnvelope(): void {
    if (this.ageInSamples < this.rcbEnvelopeAttackIndex) { // attack
      this.rcbEnvelopeAmplitude = 1.0
        + Math.cos(
          Math.PI + (Math.PI * (this.ageInSamples / this.rcbEnvelopeAttackIndex) * (this.rcbEnvelopeSustain / 2.0)),
        );
    } else if (this.ageInSamples > this.rcbEnvelopeReleaseIndex) { // release
      this.rcbEnvelopeAmplitude = 1.0
        + Math.cos(Math.PI * ((this.ageInSamples - this.rcbEnvelopeReleaseIndex) / this.rcbEnvelopeAttackIndex))
